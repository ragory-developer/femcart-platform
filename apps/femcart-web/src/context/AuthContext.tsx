  ...
  const login = (accessToken: string, refreshToken: string, userData: any) => {
    ...
    if (typeof userData.permissions === 'string') {
      try {
        ...
        Logger.warn("Failed to parse user permissions JSON during login", error, "AuthContext");
      ...
      if (res.ok && data.success) {
        ...
        if (typeof userData.permissions === 'string') {
          try {
            ...
            Logger.warn("Failed to parse user permissions JSON during profile refresh", error, "AuthContext");
            userData.permissions = [];
          }
        }
        localStorage.setItem('femcart_user', JSON.stringify(userData));
