...
export function useProductFilters() {
  ...
  useEffect(() => {
    ...
    if (attrsStr) {
      ...
      groups.forEach(group => {
        ...
        if (name && vals) {
          parsed[decodeURIComponent(name)] = vals.split(',').map(decodeURIComponent);
        }
      });
  ...
  const applyFilters = useCallback((newFilters: Partial<FilterState>) => {
    ...
    if (updated.brand.length > 0) params.set('brand', updated.brand.join(','));
    ...
    const attrEntries = Object.entries(updated.attributes)
      ...
      .map(([name, values]) => `${encodeURIComponent(name)}:${values.map(v => encodeURIComponent(v)).join(',')}`);
  ...
  const toggleAttribute = useCallback((attrName: string, attrValue: string) => {
  ...
  const setMinRating = useCallback((rating: string) => {
