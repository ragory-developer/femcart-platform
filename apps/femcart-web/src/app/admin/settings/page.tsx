...
export default function SettingsPage() {
  ...
  const [validationStatuses, setValidationStatuses] = useState<Record<string, 'idle' | 'loading' | 'valid' | 'invalid'>>({});
  ...
  const validateGateway = async (gateway: 'stripe' | 'paypal' | 'sslcz' | 'nagad' | 'bkash') => {
    ...
    if (gateway === 'stripe') {
      ...
      body = { clientId: settings.paypal_client_id, secret: settings.paypal_client_secret, isLive: settings.paypal_is_live };
    } else if (gateway === 'sslcz') {
      endpoint = '/api/global-settings/validate-sslcz';
      body = { storeId: settings.sslcz_store_id, storePassword: settings.sslcz_store_password, isLive: settings.sslcz_is_live };
    ...
    try {
      ...
      if (data.valid) {
        ...
        showToast.success(`Successfully connected to ${gateway.toUpperCase()}!`);
        ...
        showToast.error(`Invalid credentials for ${gateway.toUpperCase()}. Could not connect.`);
      }
    } catch (e) {
      setValidationStatuses(prev => ({ ...prev, [gateway]: 'invalid' }));
      showToast.error(`Network error while validating ${gateway.toUpperCase()}`);
    }
  };
          ...
          {loading ? (
            ...
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
               ...
               <section>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Configure the global look and feel of all product cards and their loading skeletons.</p>
                  
                  <div className="space-y-6 max-w-md">
                     <div className="space-y-2">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Card Variant</label>
                        <select 
                          value={settings.productCardVariant || "classic"} 
                          onChange={e => handleChange("productCardVariant", e.target.value)}
                          className="w-full px-5 py-3 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-emerald-500 transition-all font-medium text-gray-900 dark:text-white appearance-none"
                        >
                          <option value="classic">Classic (Shadows, Hover Effects)</option>
                          <option value="sleek">Sleek (Gradient, Scale)</option>
                          <option value="festive">Festive (Amber Tone, Decorative)</option>
                          <option value="minimal">Minimal (Clean, No Borders)</option>
                          <option value="bordered">Bordered (Strict Outlines)</option>
                          <option value="neumorphic">Neumorphic (Soft UI Shadows)</option>
                          <option value="horizontal">Horizontal (List View)</option>
                        </select>
                     </div>

                     <div className="space-y-2">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Corner Radius</label>
                        <select 
                          value={settings.productCardRadius || "3xl"} 
                          onChange={e => handleChange("productCardRadius", e.target.value)}
                          className="w-full px-5 py-3 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-emerald-500 transition-all font-medium text-gray-900 dark:text-white appearance-none"
                        >
                     ...
                     <div className="space-y-2">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Badge Style</label>
                        <select 
                          value={settings.productCardBadgeStyle || "pill"} 
                          onChange={e => handleChange("productCardBadgeStyle", e.target.value)}
                          className="w-full px-5 py-3 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-emerald-500 transition-all font-medium text-gray-900 dark:text-white appearance-none"
                        >
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group">
                          <div className="relative">
                            <input 
                              ...
                              onChange={e => handleChange("productCardShowBadge", e.target.checked ? "true" : "false")}
                            />
                            <div className={`w-12 h-6 rounded-full transition-colors ${settings.productCardShowBadge !== "false" ? "bg-emerald-500" : "bg-gray-200 dark:bg-gray-700"}`}></div>
                            <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${settings.productCardShowBadge !== "false" ? "translate-x-6" : ""}`}></div>
                          </div>
                          <span className="text-sm font-bold text-gray-700 dark:text-gray-200 group-hover:text-emerald-500 transition-colors uppercase tracking-tight">Show Badges (Sale/New)</span>
                        </label>
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group">
                          <div className="relative">
                            <input 
                              ...
                              onChange={e => handleChange("productCardShowRating", e.target.checked ? "true" : "false")}
                            />
                            <div className={`w-12 h-6 rounded-full transition-colors ${settings.productCardShowRating !== "false" ? "bg-emerald-500" : "bg-gray-200 dark:bg-gray-700"}`}></div>
                            <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${settings.productCardShowRating !== "false" ? "translate-x-6" : ""}`}></div>
                          </div>
                          <span className="text-sm font-bold text-gray-700 dark:text-gray-200 group-hover:text-emerald-500 transition-colors uppercase tracking-tight">Show Star Ratings</span>
                        </label>
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group">
                          <div className="relative">
                            <input 
                              type="checkbox" 
                              className="sr-only" 
                              checked={settings.productCardShowAddToCart !== "false"}
                              onChange={e => handleChange("productCardShowAddToCart", e.target.checked ? "true" : "false")}
                            />
                            <div className={`w-12 h-6 rounded-full transition-colors ${settings.productCardShowAddToCart !== "false" ? "bg-emerald-500" : "bg-gray-200 dark:bg-gray-700"}`}></div>
                            <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${settings.productCardShowAddToCart !== "false" ? "translate-x-6" : ""}`}></div>
                          </div>
                          <span className="text-sm font-bold text-gray-700 dark:text-gray-200 group-hover:text-emerald-500 transition-colors uppercase tracking-tight">Show Add to Cart Button</span>
                        </label>
                     </div>
                  </div>
               </section>
               ...
               <section>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Configure when inactive abandoned carts should be automatically deleted.</p>
                  
                  <div className="space-y-6 max-w-md">
                     <div className="space-y-2">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Auto-Cleanup Timeframe (Hours)</label>
                        <input 
                          type="number"
                          min="1"
                          value={settings.abandoned_cart_expiry_hours || "24"} 
                          onChange={e => handleChange("abandoned_cart_expiry_hours", e.target.value)}
                          ...
                          placeholder="e.g. 24"
                        />
                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider pl-1 mt-1">Automatically delete carts inactive for this many hours.</p>
                     </div>
                  </div>
               </section>
                  ...
                  <div className="space-y-6 max-w-md">
                     <div className="p-5 rounded-2xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800 space-y-4">
                         <div className="space-y-3 pb-4">
                            <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest pl-1 block">Verification Method</label>
                            <select 
                               value={settings.checkout_verification_method || "phone"} 
                               onChange={(e) => handleChange("checkout_verification_method", e.target.value)}
                         ...
                         <label className="flex items-center gap-4 cursor-pointer group pt-2">
                            ...
                            <div className="flex-1">
                               <span className="text-sm font-black text-gray-700 dark:text-gray-200 group-hover:text-emerald-500 transition-colors uppercase tracking-tight">Verify before order</span>
                               <p className="text-[10px] text-gray-500 font-medium">Require guest users to verify their contact info via OTP before placing an order.</p>
                            </div>
                         </label>
                ...
                <div className="grid gap-4">
                  <label className={`relative flex items-center p-5 rounded-2xl border-2 cursor-pointer transition-all ${
                    ...
                    <div className="flex-1">
                       ...
                       <div className="text-xs text-gray-500 mt-0.5">Example: femcart.com/apple-phone</div>
                    </div>
                  ...
                  <label className={`relative flex items-center p-5 rounded-2xl border-2 cursor-pointer transition-all ${
                    ...
                    <div className="flex-1">
                       ...
                       <div className="text-xs text-gray-500 mt-0.5">Example: femcart.com/product/apple-phone</div>
                    </div>
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
              <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-100 dark:border-gray-700 shadow-sm">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Toggle which payment methods are available during checkout.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                  {['COD', 'STRIPE', 'PAYPAL', 'BKASH', 'CARD', 'NAGAD'].map(method => (
                    <label key={method} className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl cursor-pointer border border-transparent hover:border-emerald-500/30 transition-all">
                      <div className="relative flex items-center">
                        <input 
                          type="checkbox" 
                          checked={settings[`payment_enable_${method.toLowerCase()}`] !== 'false'} // Default to true if undefined
                          onChange={e => handleChange(`payment_enable_${method.toLowerCase()}`, e.target.checked ? 'true' : 'false')}
                          className="sr-only peer"
                        />
                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-emerald-300 dark:peer-focus:ring-emerald-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-emerald-500"></div>
                      </div>
                      <span className="font-bold text-gray-900 dark:text-white">{method === 'CARD' ? 'SSL Commerz / Card' : method === 'COD' ? 'Cash on Delivery' : method}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-100 dark:border-gray-700 shadow-sm">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Configure your bKash merchant credentials.</p>
                <div className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">App Key</label>
                      <input 
                        type="text" 
                   ...
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">App Secret</label>
                      <input 
                        type="password" 
                   ...
                   <label className="flex items-center gap-3 cursor-pointer">
                      <input 
                        ...
                        onChange={e => handleChange("bkash_is_live", e.target.checked ? 'true' : 'false')}
                        className="w-5 h-5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-600"
                      />
                      <span className="font-bold text-gray-900 dark:text-white text-sm">Live Mode (Uncheck for Sandbox)</span>
                   </label>
                   <div className="pt-4 flex justify-end border-t border-gray-100 dark:border-gray-700">
                     ...
                     >
                       {validationStatuses.bkash === 'loading' ? <Loader2 className="animate-spin w-4 h-4" /> : <LinkIcon className="w-4 h-4" />}
                       Connect bKash
                     </button>
                   </div>
                </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-100 dark:border-gray-700 shadow-sm">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Configure your Stripe API keys to accept credit card payments.</p>
                <div className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Stripe Secret Key (sk_test_... or sk_live_...)</label>
                      <div className="relative">
                        <input 
                          type="password" 
                   ...
                   <div className="pt-4 flex justify-end border-t border-gray-100 dark:border-gray-700">
                     <button
                       ...
                       className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-xl font-bold transition-all shadow-md disabled:opacity-50"
                     >
                       {validationStatuses.stripe === 'loading' ? <Loader2 className="animate-spin w-4 h-4" /> : <LinkIcon className="w-4 h-4" />}
                       Connect Stripe
                     </button>
                   </div>
                </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-100 dark:border-gray-700 shadow-sm">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Configure your PayPal Client ID and Secret to accept PayPal payments.</p>
                <div className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">PayPal Client ID</label>
                      <input 
                        type="text" 
                   ...
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">PayPal Secret Key</label>
                      <input 
                        type="password" 
                   ...
                   <div className="pt-2">
                      <label className="flex items-center gap-3 cursor-pointer">
                        <input 
                          ...
                          onChange={e => handleChange("paypal_is_live", e.target.checked ? 'true' : 'false')}
                          className="w-5 h-5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-600"
                        />
                        <span className="font-bold text-gray-900 dark:text-white text-sm">Live Mode (Uncheck for Sandbox)</span>
                      </label>
                   ...
                   <div className="pt-4 flex justify-end border-t border-gray-100 dark:border-gray-700">
                     <button
                       ...
                       className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-xl font-bold transition-all shadow-md disabled:opacity-50"
                     >
                       {validationStatuses.paypal === 'loading' ? <Loader2 className="animate-spin w-4 h-4" /> : <LinkIcon className="w-4 h-4" />}
                       Connect PayPal
                     </button>
                   </div>
                </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-100 dark:border-gray-700 shadow-sm">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Configure your SSL Commerz credentials to accept Local Cards, Mobile Banking, and Net Banking.</p>
                <div className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Store ID</label>
                      <input 
                        type="text" 
                        value={settings.sslcz_store_id ?? ""} 
                        onChange={e => handleChange("sslcz_store_id", e.target.value)}
                        className="w-full px-5 py-3 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-emerald-500 transition-all font-mono text-gray-900 dark:text-white"
                        placeholder="Store ID..."
                      />
                   </div>
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Store Password</label>
                      <input 
                        type="password" 
                        value={settings.sslcz_store_password ?? ""} 
                        onChange={e => handleChange("sslcz_store_password", e.target.value)}
                        className="w-full px-5 py-3 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-emerald-500 transition-all font-mono text-gray-900 dark:text-white"
                   ...
                   <label className="flex items-center gap-3 cursor-pointer">
                      <input 
                        ...
                        onChange={e => handleChange("sslcz_is_live", e.target.checked ? 'true' : 'false')}
                        className="w-5 h-5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-600"
                      />
                      <span className="font-bold text-gray-900 dark:text-white text-sm">Live Mode (Uncheck for Sandbox)</span>
                   </label>
                   <div className="pt-4 flex justify-end border-t border-gray-100 dark:border-gray-700">
                     <button
                       ...
                       className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-xl font-bold transition-all shadow-md disabled:opacity-50"
                     >
                       {validationStatuses.sslcz === 'loading' ? <Loader2 className="animate-spin w-4 h-4" /> : <LinkIcon className="w-4 h-4" />}
                       Connect SSL Commerz
                     </button>
                   </div>
                </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 border border-gray-100 dark:border-gray-700 shadow-sm">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Configure your Nagad Merchant credentials.</p>
                <div className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Merchant ID</label>
                      <input 
                        type="text" 
                   ...
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Merchant Number</label>
                      <input 
                        type="text" 
                        value={settings.nagad_merchant_number ?? ""} 
                        onChange={e => handleChange("nagad_merchant_number", e.target.value)}
                        className="w-full px-5 py-3 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-emerald-500 transition-all font-mono text-gray-900 dark:text-white"
                        placeholder="01XXXXXXXXX"
                      />
                   </div>
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Public Key (RSA)</label>
                      <textarea 
                        value={settings.nagad_public_key ?? ""} 
                        onChange={e => handleChange("nagad_public_key", e.target.value)}
                        rows={4}
                   ...
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Private Key (RSA)</label>
                      <textarea 
                        value={settings.nagad_private_key ?? ""} 
                        onChange={e => handleChange("nagad_private_key", e.target.value)}
                        rows={4}
                        className="w-full px-5 py-3 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-emerald-500 transition-all font-mono text-xs text-gray-900 dark:text-white"
                   ...
                   <label className="flex items-center gap-3 cursor-pointer">
                      <input 
                        ...
                        onChange={e => handleChange("nagad_is_live", e.target.checked ? 'true' : 'false')}
                        className="w-5 h-5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-600"
                      />
                      <span className="font-bold text-gray-900 dark:text-white text-sm">Live Mode (Uncheck for Sandbox)</span>
                   </label>
                   <div className="pt-4 flex justify-end border-t border-gray-100 dark:border-gray-700">
                     <button
                       ...
                       className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-xl font-bold transition-all shadow-md disabled:opacity-50"
                     >
                       {validationStatuses.nagad === 'loading' ? <Loader2 className="animate-spin w-4 h-4" /> : <LinkIcon className="w-4 h-4" />}
                       Connect Nagad
                     </button>
                   </div>
                </div>
