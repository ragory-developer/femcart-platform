...
export default function AdminLayout({
  ...
  if (user) {
    if (user.role === 'SUPER_ADMIN') {
      ...
      hasPermission = perms.includes('ALL') || perms.includes(requiredPermission);
    }
  }
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
          ...
          <p className="text-gray-500 max-w-md mx-auto">
            You do not have the required permissions (<strong className="text-gray-300">{requiredPermission}</strong>) to view this page.
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-50 dark:bg-gray-950 font-sans">
      ...
      <div className={`${collapsed ? "ml-[80px]" : "ml-[280px]"} min-h-[100dvh] transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]`}>
        <main className={isBuilderPage ? "p-0" : "p-6 lg:p-8"}>{renderContent()}</main>
      </div>
    </div>
  );
}
