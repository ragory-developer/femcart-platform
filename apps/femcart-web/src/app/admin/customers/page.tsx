...
export default function CustomersPage() {
  ...
  const totalSpent = filteredCustomers.reduce((sum, c) => sum + (c.totalOrderAmount || 0), 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              ...
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">${totalSpent.toFixed(2)}</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">By filtered customers</p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              ...
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">{registeredCount}</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Full account holders</p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Guest Shoppers</p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">{guestCount}</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Checked out without account</p>
           </div>
        </div>
      </div>
           ...
           <table className="w-full text-left border-collapse min-w-[800px] text-sm">
             <thead>
               <tr className="bg-gray-50/80 dark:bg-gray-800/80">
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 w-1/3">Customer Info</th>
                 ...
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-center">Total Spent</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-right">Joined / Location</th>
               ...
               {loading ? (
                 ...
                 filteredCustomers.map((customer) => (
                   <React.Fragment key={customer.id}>
                     <tr className={`group transition-colors duration-200 cursor-pointer hover:bg-gray-50/80 dark:hover:bg-gray-750/30 ${expandedId === customer.id ? 'bg-blue-50/40 dark:bg-blue-900/10' : ''}`} onClick={() => setExpandedId(expandedId === customer.id ? null : customer.id)}>
                         ...
                         <div className="flex items-center gap-3">
                           ...
                           <div>
                             <div className="flex items-center gap-2">
                               <p className="font-bold text-gray-900 dark:text-white">{customer.name || "Unknown"}</p>
                             </div>
                       ...
                       <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750 text-right">
                         <div className="flex flex-col items-end gap-1">
                           ...
                           {(customer.area || customer.city) && (
                             <div className="flex items-center gap-1.5 text-gray-500 dark:text-gray-400 font-medium text-xs">
                               <MapPin size={12} />
                               {customer.area ? `${customer.area}, ${customer.city}` : customer.city}
                             </div>
                           )}
                         </div>
                       </td>
