...
import { Calendar, Eye, Mail, MailOpen, Phone, Search, Trash2, X } from "lucide-react";
...
export default function ContactMessagesPage() {
  ...
  const fetchMessages = async () => {
    try {
      setLoading(true);
      const url = new URL(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/contact`);
  ...
  const toggleReadStatus = async (id: string, currentStatus: boolean) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/contact/${id}/read`, {
        method: 'PATCH',
        headers: {
  ...
  const deleteMessage = async (id: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/contact/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1400px] mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight">Contact Messages</h1>
          <p className="text-gray-500 dark:text-gray-400 font-medium mt-2">Manage customer inquiries and feedback.</p>
        </div>
      </div>
      ...
      <div className="glass p-6 md:p-8 rounded-3xl border border-gray-200/50 dark:border-gray-800/50 shadow-xl shadow-emerald-500/5">
        ...
        {loading ? (
          ...
          <div className="py-20 text-center">
            ...
            <p className="text-gray-500">We couldn't find any contact messages matching your criteria.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-[800px] text-sm">
              <thead>
                <tr className="bg-gray-50/80 dark:bg-gray-800/80">
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Status</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Date</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Name</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Contact Info</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Subject</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-right">Actions</th>
                </tr>
              </thead>
              <tbody>
                {messages.map((msg) => (
                  <tr 
                    ...
                    className={`group transition-colors duration-200 hover:bg-gray-50/80 dark:hover:bg-gray-750/30 cursor-pointer ${
                      !msg.isRead ? "bg-emerald-50/30 dark:bg-emerald-900/10 font-medium" : ""
                    }`}
                    onClick={() => {
                  ...
                  >
                    ...
                    <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-gray-900 dark:text-white font-bold text-sm">{msg.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750 whitespace-nowrap text-gray-600 dark:text-gray-400">
                      {msg.email}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/50">
              <h2 className="text-xl font-black text-gray-900 dark:text-white">Message Details</h2>
              <button 
            ...
            <div className="p-6 md:p-8 overflow-y-auto flex-1 space-y-6">
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 bg-emerald-50/50 dark:bg-emerald-900/10 p-5 rounded-2xl border border-emerald-100 dark:border-emerald-800/30">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900 dark:text-white text-lg">{selectedMessage.name}</h3>
                    <p className="text-sm font-medium text-emerald-600 dark:text-emerald-400">{selectedMessage.email}</p>
                  </div>
                </div>
              ...
              <div>
                <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Subject</h4>
                <p className="text-lg font-black text-gray-900 dark:text-white">{selectedMessage.subject}</p>
              </div>
            ...
            <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 flex flex-col md:flex-row justify-between gap-4">
              <div className="flex gap-3">
                <a 
                  href={isPhoneNumber(selectedMessage.email) ? `tel:${selectedMessage.email}` : `mailto:${selectedMessage.email}`}
                ...
                >
                  {isPhoneNumber(selectedMessage.email) ? <Phone size={16} /> : <Mail size={16} />}
                  Contact Customer
                </a>
