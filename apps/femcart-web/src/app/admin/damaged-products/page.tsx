...
export default function DamagedProductsPage() {
  ...
  const [restockItem, setRestockItem] = useState<{ id: string, maxQty: number, productName: string } | null>(null);
  ...
  const handleConfirmRestock = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("femcart_access_token") || localStorage.getItem("token") || "";
      const res = await fetch(`${API_URL}/api/orders/damaged-items/${restockItem.id}/restock`, {
        method: "POST",
        headers: { 
  ...
  const totalDamagedValue = filteredItems.reduce((sum, item) => sum + (item.price * item.damagedQuantity), 0);
  const totalItemsDamaged = filteredItems.reduce((sum, item) => sum + item.damagedQuantity, 0);
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Total Items Damaged</p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">{totalItemsDamaged} Units</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Pending review or write-off</p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Total Value Lost</p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">${totalDamagedValue.toFixed(2)}</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Estimated loss</p>
           </div>
        </div>
      </div>
           ...
           <table className="w-full text-left border-collapse min-w-[800px] text-sm">
             <thead>
               <tr className="bg-gray-50/80 dark:bg-gray-800/80">
                 ...
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Damaged Product</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-center">Qty</th>
                 ...
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-right">Time</th>
             ...
             <tbody>
               {loading ? (
                 <tr>
                   <td colSpan={6} className="py-8 text-center text-gray-500 font-medium">Loading damaged items...</td>
                 </tr>
                 ...
                 <tr>
                   <td colSpan={6} className="py-8">
                     <div className="flex flex-col items-center justify-center gap-3 text-gray-400">
                       ...
                       <p className="font-medium text-gray-500">No damaged items found.</p>
                     </div>
                   </td>
                 </tr>
               ) : (
                 filteredItems.map((item) => (
                   <tr key={item.id} className="group transition-colors duration-200 hover:bg-gray-50/80 dark:hover:bg-gray-750/30">
                     ...
                     <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750 text-center">
                        ...
                        >
                          <RefreshCw size={14} className={restockingId === item.id ? "animate-spin" : ""} />
      ...
      {restockModalOpen && restockItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-gray-900/40 backdrop-blur-sm" onClick={() => !restockingId && setRestockModalOpen(false)} />
          <div className="bg-white dark:bg-gray-900 rounded-[2rem] w-full max-w-md shadow-2xl z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 text-white flex items-center gap-4 bg-emerald-500">
               ...
               <div>
                  <h3 className="font-black text-xl uppercase tracking-widest">Restock Item</h3>
                  <p className="text-white/80 text-sm font-medium line-clamp-1">{restockItem.productName}</p>
               </div>
            </div>
            
            <div className="p-8">
              <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-3">Quantity to Restock</label>
