...
import { Image as ImageIcon, Pencil, Plus, Search, Tag, Trash2, X } from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">Manage and organize your product brands</p>
        </div>
        {!showForm && (
           <button
      ...
      <div className="flex flex-col lg:flex-row-reverse gap-8">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 p-8 shadow-2xl shadow-emerald-500/5 sticky top-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider pl-1 font-outfit">Brand Name</label>
                  <input
                    type="text"
                    required
                    value={name}
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider pl-1 font-outfit">Brand Story</label>
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider pl-1 font-outfit">Brand Logo</label>
                  <div 
                  ...
                  >
                    {logo ? (
                      <>
                        <img src={logo} className="w-full h-full object-contain bg-white dark:bg-gray-900 p-4" alt="Brand Representative" />
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                           <div className="flex flex-col items-center gap-1 scale-90 group-hover:scale-100 transition-transform">
                              ...
                              <span className="text-white font-black uppercase tracking-[0.2em] text-[9px]">Change Identity</span>
                           </div>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <span className="text-gray-400 font-black text-[9px] uppercase tracking-widest text-center px-4">Click to Upload Brand Visual</span>
                      </>
                    )}
                  </div>
                </div>
        ...
        <div className="flex-1 bg-white dark:bg-gray-900 rounded-[3rem] border border-gray-100 dark:border-gray-800 p-8 shadow-sm">
           <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white tracking-tight">Brands Overview</h3>
                <p className="text-xs text-gray-500">Manage your brand portfolio</p>
              </div>
           ...
           <div className="grid gap-4">
             {loading ? (
                <div className="py-20 text-center text-gray-400 font-medium">Summoning brands...</div>
             ) : filteredBrands.length === 0 ? (
                <div className="py-20 text-center">
                   <Tag size={48} className="mx-auto text-gray-200 mb-4" />
                   <p className="text-gray-400 font-medium tracking-tight">No brands found.</p>
                </div>
             ) : (
                filteredBrands.map(brand => (
                  <div key={brand.id} className="group flex items-center justify-between p-4 rounded-3xl border border-gray-50 dark:border-gray-800 hover:border-emerald-100 dark:hover:border-emerald-900/30 hover:bg-emerald-50/20 dark:hover:bg-emerald-900/5 transition-all">
                    <div className="flex items-center gap-4">
                      {brand.logo && brand.logo !== "null" && !imgError[brand.id] ? (
                        <img 
                          src={brand.logo} 
                          alt={brand.name} 
                          ...
                          onError={() => setImgError(prev => ({ ...prev, [brand.id]: true }))}
                        />
                      ) : (
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                           <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">/{brand.slug}</span>
                           <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                           <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">{brand._count?.products ?? 0} ITEMS</span>
                        </div>
                      </div>
                    </div>
