...
export default function SmsMarketingPage() {
  ...
  const filteredCustomers = customers.filter(c => {
    ...
    if (search) {
      ...
      if (!c.name?.toLowerCase().includes(s) && !c.phone?.includes(s) && !c.email?.toLowerCase().includes(s)) {
        return false;
      }
    }
  ...
  const getParsedMessage = (customer: Customer | undefined, template: string) => {
    if (!customer) return "";
    let msg = template || (smsType === 'COUPON' ? "Hi [Name], check out our latest offers: [StoreLink]" : "Hi [Name], we have an important update: [StoreLink]");
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-0.5">Total Audience</p>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{customers.length}</h3>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-0.5">Registered Users</p>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{customers.filter(c => !c.isGuest).length}</h3>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-0.5">Guest Shoppers</p>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{customers.filter(c => c.isGuest).length}</h3>
          </div>
        </div>
      </div>
           ...
           <table className="w-full text-left border-collapse min-w-[800px] text-sm">
             <thead>
               <tr className="bg-gray-50/80 dark:bg-gray-800/80">
                 ...
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 w-1/3">Customer</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-center">Orders</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-center">Spent</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-center">Type</th>
               </tr>
             </thead>
             <tbody>
               {loading ? (
                 <tr>
                   <td colSpan={5} className="py-8 text-center text-gray-500 font-medium">Loading customers...</td>
                 </tr>
                 ...
                 filteredCustomers.map((customer) => (
                   <tr key={customer.id} className="group transition-colors duration-200 hover:bg-gray-50/80 dark:hover:bg-gray-750/30">
                     ...
                     <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750">
                       <div className="flex items-center gap-3">
                         <div className="w-8 h-8 rounded-full bg-gray-100 text-gray-500 dark:bg-gray-800 flex items-center justify-center shrink-0">
                           {customer.isGuest ? <UserX size={14} /> : <Users size={14} />}
                         </div>
                         <div>
                           <p className="font-semibold text-gray-900 dark:text-white text-sm">{customer.name || "Unknown"}</p>
      ...
      <SmsCouponModal 
        ...
        previewMessage={getParsedMessage(filteredCustomers.find(c => c.id === selectedCustomerIds[0]), smsMessage)}
        onSend={async () => {
          const selected = filteredCustomers.filter(c => selectedCustomerIds.includes(c.id));
          const payloads = selected.map(customer => ({
