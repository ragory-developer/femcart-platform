...
const emptyForm = { name: "", email: "", password: "", role: "ADMIN" as "ADMIN" | "SUPER_ADMIN", permissions: [] as string[], adminRoleId: "" };
...
export default function AdminUsersPage() {
  ...
  const token = typeof window !== "undefined" ? localStorage.getItem("femcart_access_token") : "";
  const getToken = () => (typeof window !== "undefined" ? localStorage.getItem("femcart_access_token") || "" : "");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [userRes, roleRes] = await Promise.all([
        fetch(`${API_URL}/api/admin-users`, { headers: { Authorization: `Bearer ${tokenStr}` } }),
        fetch(`${API_URL}/api/admin-roles`, { headers: { Authorization: `Bearer ${tokenStr}` } })
          .catch(() => ({ json: async () => ({ success: false, data: [] }), ok: false }))
      ]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setEditTarget(null); setForm(emptyForm); fetchData();
    } catch (e: any) { setError(e.message); } finally { setSaving(false); }
  };
          ...
          <table className="w-full text-left border-collapse min-w-[800px] text-sm">
            <thead>
              <tr className="bg-gray-50/80 dark:bg-gray-800/80">
                <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-left">User</th>
                <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-left">Role</th>
                <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-left">Permissions</th>
            ...
            <tbody>
              {admins.map((admin) => (
                <tr key={admin.id} className="group transition-colors duration-200 hover:bg-gray-50/80 dark:hover:bg-gray-750/30">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-gray-950 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-2xl p-8 max-h-[90vh] overflow-y-auto [&::-webkit-scrollbar]:hidden">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Access Level</label>
              ...
              {form.role === 'ADMIN' && (
                <>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Custom Role</label>
                    <select value={form.adminRoleId} onChange={e => setForm(f => ({ ...f, adminRoleId: e.target.value }))}
                      className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-gray-900 dark:text-white">
