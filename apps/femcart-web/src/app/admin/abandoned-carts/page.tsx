...
export default function AbandonedCartsPage() {
  ...
  const filteredCarts = carts.filter(cart => {
    ...
    if (dateFilter !== 'ALL') {
      ...
      const now = new Date();
      const diffInDays = (now.getTime() - cartDate.getTime()) / (1000 * 3600 * 24);
  ...
  const toggleSelectAll = () => {
    if (selectedCartIds.length === filteredCarts.length && filteredCarts.length > 0) {
      setSelectedCartIds([]);
    } else {
  ...
  const getParsedMessage = (cart: Cart | undefined, template: string) => {
    if (!cart) return "";
    let msg = template || (smsType === 'COUPON' ? "Hi [Name], we noticed you left items in your cart! Use code SAVE10 for 10% off." : "Hi [Name], you left items in your Femcart. Come back and checkout!");
    
    // Parse placeholders
    msg = msg.replace(/\[Name\]/g, cart.user?.name || "Customer");
    
    // Parse CartItems
    const itemsList = cart.items.map(i => `${i.quantity}x ${i.product?.name}`).join(', ');
    msg = msg.replace(/\[CartItems\]/g, itemsList);
    
    // Generate a checkout link (could be customized per user/cart)
    const domain = typeof window !== 'undefined' ? window.location.origin : 'https://femcart.com';
  ...
  const totalAbandonedValue = filteredCarts.reduce((sum, cart) => sum + (cart.totalValue || 0), 0);
  ...
  const insertPlaceholder = (placeholder: string) => {
    setSmsMessage((prev) => prev + (prev.endsWith(' ') || prev.length === 0 ? '' : ' ') + placeholder);
  };
    ...
    <div className="space-y-6 pb-10">
      ...
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Track and analyze carts where users added items but failed to checkout.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Potential Value Lost</p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">${totalAbandonedValue.toFixed(2)}</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Total value of carts shown</p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Guest Abandonments</p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">{guestCartsCount} Carts</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Users without accounts</p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Registered Abandonments</p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">{registeredCartsCount} Carts</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Logged in users</p>
           </div>
        </div>
      </div>
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-gray-100 dark:border-gray-800">
           ...
           <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
             <div className="relative w-full sm:w-auto">
               <select 
                 value={dateFilter}
                 onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setDateFilter(e.target.value as 'ALL' | 'TODAY' | 'LAST_7' | 'LAST_30')}
           ...
           <div className="flex items-center gap-2">
             <button 
               ...
               onClick={() => { setSmsType('SMS'); setIsSmsModalOpen(true); }}
             ...
             >
               <MessageSquare size={16} /> Send SMS {selectedCartIds.length > 0 && `(${selectedCartIds.length})`}
             </button>
             <button 
               disabled={selectedCartIds.length === 0}
               onClick={() => { setSmsType('COUPON'); setIsSmsModalOpen(true); }}
             ...
             >
               <Ticket size={16} /> Send Coupon {selectedCartIds.length > 0 && `(${selectedCartIds.length})`}
             </button>
           </div>
         </div>
           ...
           <table className="w-full text-left border-collapse min-w-[800px] text-sm">
             <thead>
               <tr className="bg-gray-50/80 dark:bg-gray-800/80">
                 ...
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Customer</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Items Summary</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-right">Cart Value</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-right">Time Abandoned</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-center">Details</th>
               </tr>
             </thead>
             <tbody>
               {loading ? (
                 <tr>
                   <td colSpan={6} className="py-8 text-center text-gray-500 font-medium">Loading abandoned carts...</td>
                 </tr>
               ) : filteredCarts.length === 0 ? (
                 <tr>
                   <td colSpan={6} className="py-8">
                     <div className="flex flex-col items-center justify-center gap-3 text-gray-400">
                       <PackageOpen size={48} className="text-gray-300 dark:text-gray-700"/>
                       <p className="font-medium text-gray-500">No abandoned carts found matching filters.</p>
                     </div>
                   </td>
                 </tr>
               ) : (
                 filteredCarts.map((cart) => (
                   <React.Fragment key={cart.id}>
                     <tr className={`group transition-colors duration-200 hover:bg-gray-50/80 dark:hover:bg-gray-750/30 cursor-pointer ${expandedCartId === cart.id ? 'bg-rose-50/80 dark:bg-rose-900/20' : ''}`} onClick={() => setExpandedCartId(expandedCartId === cart.id ? null : cart.id)}>
                       ...
                       <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750">
                         <div className="flex items-center gap-3">
                           ...
                           <div>
                             <div className="flex items-center gap-2">
                               <p className="font-bold text-gray-900 dark:text-white">{cart.user?.name || "Unknown"}</p>
                               {cart.user?.isGuest ? (
                                 <span className="bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 px-1.5 py-0.5 rounded-[4px] text-[9px] font-black uppercase tracking-wider">GUEST</span>
                               ) : (
                                 <span className="bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 px-1.5 py-0.5 rounded-[4px] text-[9px] font-black uppercase tracking-wider">USER</span>
                               )}
                             </div>
                             {(cart.user?.email || cart.user?.phone) && (
                               <p className="text-xs text-gray-500 mt-0.5">
                                 {cart.user?.isGuest ? cart.user?.phone : (cart.user?.email || cart.user?.phone)}
                               </p>
                             )}
                           </div>
                         </div>
                       ...
                       <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750">
                         <p className="font-bold text-gray-800 dark:text-gray-200">
                           {cart.items.reduce((sum, item) => sum + item.quantity, 0)} items <span className="font-medium text-gray-400 text-xs">({cart.items.length} unique)</span>
                         </p>
                       ...
                       <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750 text-center">
                         <button 
                           onClick={(e) => { e.stopPropagation(); setExpandedCartId(expandedCartId === cart.id ? null : cart.id); }}
                         ...
                         >
                           {expandedCartId === cart.id ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                         </button>
                       </td>
                     </tr>
                     ...
                     {expandedCartId === cart.id && (
                       <tr className="bg-rose-50/50 dark:bg-rose-900/10">
                         <td colSpan={6} className="p-0 border-b border-rose-100 dark:border-rose-900/30">
                           <div className="px-6 py-5 shadow-inner">
                             <div className="flex items-center justify-between mb-4">
                               <h4 className="text-sm font-black text-gray-800 dark:text-gray-200">Cart Contents</h4>
                             </div>
                             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                               {cart.items.map((item) => (
                                 <div key={item.id} className="flex items-center gap-4 bg-white dark:bg-gray-800 p-3 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                                   ...
                                   <div className="flex-1 min-w-0">
                                     <p className="text-sm font-bold text-gray-900 dark:text-white line-clamp-1">{item.product?.name || "Unknown Product"}</p>
                                     <div className="flex items-center gap-2 mt-1">
                                        <span className="text-xs font-black text-rose-600 dark:text-rose-400">${item.product?.price || 0}</span>
                                        <span className="text-[10px] font-bold text-gray-400 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">Qty: {item.quantity}</span>
                                     </div>
                                   </div>
                                 </div>
                               ))}
      ...
      <SmsCouponModal 
        ...
        previewMessage={getParsedMessage(filteredCarts.find(c => c.id === selectedCartIds[0]), smsMessage)}
        onSend={() => {
          const selected = filteredCarts.filter(c => selectedCartIds.includes(c.id));
          ...
          alert(`Successfully parsed and sent messages to ${selectedCartIds.length} users!\n\nOpen your browser console to see the exact parsed payloads.`);
