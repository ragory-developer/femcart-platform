...
import { FolderTree, Image as ImageIcon, Pencil, Plus, Search, Trash2, X, ChevronRight, GripVertical, Network } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast.error(err.message || "Failed to save data. Please check all fields.");
      }
    } catch (err) { 
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col justify-center ml-2">
                    <div className="flex items-center gap-2">
                      ...
                      {cat.seoData?.title && (
                          <span className="bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 text-[9px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-wider">SEO</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                       <span className="text-[11px] font-medium text-gray-400">/{cat.slug}</span>
                       <span className="w-1 h-1 rounded-full bg-gray-200 dark:bg-gray-700"></span>
                       <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">{getProductCount(cat)} ITEMS</span>
                    </div>
                  </div>
                </div>
             ...
             {loading ? (
                ...
                <div className="grid gap-3">
                  {filteredFlatList.length === 0 ? (
                    ...
                    renderTree(filteredFlatList.map(c => ({ ...c, children: [] })))
                  )}
                </div>
             ) : categories.length === 0 ? (
                <div className="py-20 text-center">
