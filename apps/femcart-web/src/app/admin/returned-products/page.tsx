...
const getFirstImage = (imagesData: any) => {
  ...
  if (typeof imagesData === 'string') {
    try {
      ...
      if (imagesData.startsWith('http') || imagesData.startsWith('/')) return imagesData;
      return null;
    }
  }
  return null;
...
export default function ReturnedProductsPage() {
  ...
  const [triageItem, setTriageItem] = useState<{ id: string, maxQty: number, productName: string } | null>(null);
  const [triageAction, setTriageAction] = useState<'RESTOCK' | 'DAMAGED'>('RESTOCK');
  ...
  const fetchItems = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/orders/returned-items?limit=100`, {
  ...
  const openTriageModal = (item: OrderItem, action: 'RESTOCK' | 'DAMAGED', maxQuantity: number) => {
    const productName = (item.product || item.variant?.product)?.name || "Unknown Product";
  ...
  const handleConfirmTriage = async () => {
    ...
    try {
      const token = localStorage.getItem("femcart_access_token") || localStorage.getItem("token") || "";
      const res = await fetch(`${API_URL}/api/orders/returned-items/${triageItem.id}/triage`, {
        method: 'POST',
        headers: { 
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ action: triageAction, quantity: triageQuantity })
      });
      const data = await res.json();
      if (data.success) {
  ...
  const filteredItems = items.filter(item => {
    if (searchProduct) {
      ...
      const matchName = actualProduct?.name?.toLowerCase().includes(searchProduct.toLowerCase());
      const matchOrder = item.order?.id?.toLowerCase().includes(searchProduct.toLowerCase());
      if (!matchName && !matchOrder) return false;
    }
    return true;
  });

  const totalReturnedValue = filteredItems.reduce((sum, item) => sum + (item.price * item.returnedQuantity), 0);
  const totalItemsReturned = filteredItems.reduce((sum, item) => sum + item.returnedQuantity, 0);
    ...
    <div className="space-y-6 pb-10">
      ...
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Track and triage physical items returned by customers. Mark them for restock or as damaged.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Pending Triage</p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">{totalItemsReturned} Units</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Awaiting your action</p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-4">
           ...
           <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Pending Value</p>
              <h3 className="text-2xl font-black text-gray-900 dark:text-white">${totalReturnedValue.toFixed(2)}</h3>
              <p className="text-xs font-bold text-gray-400 mt-1">Value of pending items</p>
           </div>
        </div>
      </div>
           ...
           <table className="w-full text-left border-collapse min-w-[800px] text-sm">
             <thead>
               <tr className="bg-gray-50/80 dark:bg-gray-800/80">
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Order & Customer</th>
                 <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Product Returned</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-center">Pending Qty</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-right">Value</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 text-center">Actions</th>
                </tr>
             </thead>
             <tbody>
               {loading ? (
                 <tr>
                   <td colSpan={5} className="py-8 text-center text-gray-500 font-medium">Loading returned items...</td>
                 </tr>
                 ...
                 <tr>
                   <td colSpan={5} className="py-8">
                     <div className="flex flex-col items-center justify-center gap-3 text-gray-400">
                       ...
                       <p className="font-medium text-gray-500">No returned items found.</p>
                     </div>
                   </td>
                 </tr>
               ) : (
                 filteredItems.map((item) => (
                   <tr key={item.id} className="group transition-colors duration-200 hover:bg-gray-50/80 dark:hover:bg-gray-750/30">
                       ...
                       <div className="flex items-center gap-3">
                         ...
                         <div>
                           <div className="flex items-center gap-2">
                             <p className="font-bold text-gray-900 dark:text-white">{item.order?.user?.name || "Customer"}</p>
                           </div>
                     ...
                     <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750">
                          ...
                          <div className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-800 overflow-hidden shrink-0 border border-gray-200 dark:border-gray-700">
                            {(() => {
                              ...
                              return img ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center text-gray-400"><PackageOpen size={16}/></div>
                              );
                            })()}
                          </div>
                         <p className="font-bold text-gray-800 dark:text-gray-200 line-clamp-1">{(item.product || item.variant?.product)?.name || "Unknown Product"}</p>
                       </div>
                     </td>
                      <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750 text-center font-black text-gray-700 dark:text-gray-300 text-lg">
                        {item.returnedQuantity - (item.restockedQuantity || 0) - (item.damagedQuantity || 0)}
                      </td>
                      <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750 font-black text-emerald-600 dark:text-emerald-400 text-right text-lg">
                        ${(item.price * (item.returnedQuantity - (item.restockedQuantity || 0) - (item.damagedQuantity || 0))).toFixed(2)}
                      </td>
                      <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750 text-center">
                        <div className="flex justify-center gap-2">
                           <button 
                             onClick={() => openTriageModal(item, 'RESTOCK', item.returnedQuantity - (item.restockedQuantity || 0) - (item.damagedQuantity || 0))}
                           ...
                           <button 
                             onClick={() => openTriageModal(item, 'DAMAGED', item.returnedQuantity - (item.restockedQuantity || 0) - (item.damagedQuantity || 0))}
      ...
      {triageModalOpen && triageItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-gray-900/40 backdrop-blur-sm" onClick={() => !triageProcessing && setTriageModalOpen(false)} />
          <div className="bg-white dark:bg-gray-900 rounded-[2rem] w-full max-w-md shadow-2xl z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className={`p-6 text-white flex items-center gap-4 ${triageAction === 'RESTOCK' ? 'bg-emerald-500' : 'bg-rose-500'}`}>
               <div className="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center shrink-0">
                 {triageAction === 'RESTOCK' ? <RotateCcw size={24} /> : <PackageMinus size={24} />}
               </div>
               <div>
                  <h3 className="font-black text-xl uppercase tracking-widest">{triageAction === 'RESTOCK' ? 'Restock Item' : 'Mark Damaged'}</h3>
                  <p className="text-white/80 text-sm font-medium line-clamp-1">{triageItem.productName}</p>
               </div>
            </div>
            
            <div className="p-8">
              <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-3">Quantity to {triageAction}</label>
              <div className="relative">
                 <input 
                   ...
                   onChange={e => setTriageQuantity(parseInt(e.target.value) || 1)}
