...
export default function AdminMediaPage() {
      ...
      try {
        ...
        if (json.success) {
          ...
          setItems((prev) => {
            ...
            const uniqueItems = Array.from(new Map(combined.map((item: MediaItem) => [item.id, item])).values());
  ...
  const handleSelect = (item: MediaItem) => {
    setSelectedItems((prev) => {
      ...
      const newItems = isSelected ? prev.filter((i) => i.id !== item.id) : [...prev, item];
    ...
    <div {...getRootProps()} className="min-h-[100dvh] relative">
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0">
          {loading ? (
          ...
          ) : items.filter(i => !pendingDeletions.includes(i.id)).length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
                {items.filter(i => !pendingDeletions.includes(i.id)).map((item) => (
                  <button
                    key={item.id}
