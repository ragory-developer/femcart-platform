...
import { Image as ImageIcon, ImagePlus, Link as LinkIcon, X } from "lucide-react";
...
export default function MediaTab() {
  ...
  const handleMediaSelect = (media: any, url: string | string[]) => {
    if (activeTarget === "main") {
      setValue("image", Array.isArray(url) ? url[0] : url, { shouldValidate: true });
    ...
    <div className="max-w-4xl space-y-8 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 md:p-8 shadow-sm">
        <div className="border-b border-gray-100 dark:border-gray-750 pb-4 mb-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Main Product Image</h2>
          <p className="text-sm text-gray-500 mt-1">This is the primary image displayed on the product page.</p>
        </div>
            <div className="flex items-center gap-4">
              ...
              >
                {image ? (
                  <img src={resolveImageUrl(image)} alt="Main" className="w-full h-full object-cover pointer-events-none" />
                ) : (
                  <>
                    <ImageIcon className="text-gray-450 dark:text-gray-500 mb-2" size={24} />
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 md:p-8 shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-750 pb-4 mb-6">
          <div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">Gallery Images</h3>
        ...
        {galleryImages.length === 0 ? (
          ...
          >
            <ImagePlus size={28} />
            <span className="text-sm font-bold">Click to add gallery images</span>
          </button>
        ) : (
