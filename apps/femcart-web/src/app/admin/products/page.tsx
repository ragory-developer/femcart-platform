...
import { Image as ImageIcon, LayoutGrid, PackageOpen, Pencil, Plus, Search, Trash2, Star } from "lucide-react";
...
export default function AdminProductsPage() {
  ...
  const columns: DataTableColumn<Product>[] = [
    {
      ...
      render: (product) => (
        ...
        >
          {product.image ? (
            <img src={resolveImageUrl(product.image)} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
          ) : (
    ...
    },
    {
      ...
      thClassName: "w-[18%]",
      render: (product) => {
        const catText = product.categories.length > 0 ? product.categories.map(c => c.name).join(", ") : "";
        return (
          <p 
      ...
      render: (product) => (
        <div className="flex items-center gap-2">
          <button
            ...
            title={product.featured ? "Featured Product. Click to unfeature" : "Click to feature this product"}
          >
      ...
      render: (product) => {
        ...
        return (
          <div className="flex items-center justify-end gap-2">
            {isMultipleSelected ? (
              <button disabled className="p-2 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 text-gray-300 dark:text-gray-600 cursor-not-allowed opacity-50" title="Edit disabled when multiple selected">
                <Pencil size={15} />
              </button>
            ) : (
              <Link
  ...
  const renderMobileCard = (product: Product, isSelected: boolean, onToggleSelect: () => void) => {
    return (
      <div className={`bg-white dark:bg-gray-800 rounded-2xl border p-4 space-y-3.5 shadow-sm transition-all duration-300 relative overflow-hidden ${
        isSelected 
          ...
          <div className="flex items-center gap-2">
            ...
            {(() => {
              ...
              return isMultipleSelected ? (
                <button disabled className="p-1.5 rounded-lg bg-gray-50/50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 text-gray-300 dark:text-gray-600 cursor-not-allowed opacity-50">
                  <Pencil size={14} />
                </button>
              ) : (
                <Link
  ...
  const toggleFeatured = async (id: string, currentFeatured: boolean) => {
    try {
      ...
      if (json.success) {
        ...
        showToast.success(`Product ${currentFeatured ? 'marked as featured' : 'removed from featured'}`);
      } else {
    ...
    if (isOnSale) {
      return (
        <div className="flex flex-col">
          <span className="text-emerald-600 dark:text-emerald-400 font-bold font-mono text-sm">${p.specialPrice?.toFixed(2)}</span>
          <span className="text-xs text-gray-400 line-through font-mono">${p.price.toFixed(2)}</span>
        </div>
      );
    }
  ...
  const visibleProducts = products.filter(p => !pendingDeletions.includes(p.id));
    ...
    <div className="max-w-7xl mx-auto pb-12 px-4 sm:px-6">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-405 mt-1 text-sm">Create, manage, and monitor all your inventory items</p>
        </div>
        <Link
          ...
          {selectedProducts.length > 0 ? (
            <div className="flex flex-1 items-center justify-between w-full animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center gap-4">
                ...
                <span className="font-semibold text-gray-700 dark:text-gray-200">Products Selected</span>
              </div>
              <div className="flex items-center gap-2">
                <button
          ...
          <DataTable
            ...
            emptyState={
              <div className="flex flex-col items-center justify-center py-20 text-gray-500 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-750 rounded-2xl">
                <PackageOpen size={48} className="mx-auto mb-3 opacity-20 text-gray-400" />
        ...
        {totalPages > 1 && (
          <div className="p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-750 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 shadow-sm">
            <span className="text-sm text-gray-500 dark:text-gray-400 font-medium order-2 sm:order-1">
              Showing page <span className="font-bold text-gray-700 dark:text-gray-200">{page}</span> of <span className="font-bold text-gray-700 dark:text-gray-200">{totalPages}</span>
            </span>
