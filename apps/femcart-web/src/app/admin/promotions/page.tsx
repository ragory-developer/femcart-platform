...
import { Calculator, Calendar, CheckCircle2, Image as ImageIcon, PackageOpen, Percent, Search, X } from "lucide-react";
...
export default function PromotionsPage() {
  ...
  const handleApplyPromotion = async (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedIds.size === 0) return showToast.error("Select at least one product.");
    if (!promoValue || isNaN(Number(promoValue)) || Number(promoValue) <= 0) return showToast.error("Enter a valid discount value.");
          ...
          <table className="w-full text-left border-collapse min-w-[800px] text-sm">
            <thead className="sticky top-0 bg-gray-50/80 dark:bg-gray-800/80 z-10 backdrop-blur-sm">
              <tr>
                ...
                <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 w-[10%]">Product</th>
                <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 w-[35%]">Name & Categories</th>
                <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 w-[15%]">Price</th>
                <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700 w-[15%]">Status</th>
              </tr>
            </thead>
            <tbody>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750">
                        <div 
                          ...
                          onClick={() => window.open(settings?.permalink_structure === 'product' ? `/product/${product.slug}` : `/${product.slug}`, '_blank')}
                        >
                          {product.image ? (
