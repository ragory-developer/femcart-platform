...
export default function AdminReviewsPage() {
  ...
  const getToken = () => (typeof window !== "undefined" ? localStorage.getItem("femcart_access_token") || localStorage.getItem("token") || "" : "");
  ...
  const fetchReviews = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(url, { headers: { Authorization: `Bearer ${tokenStr}` } });
  ...
  const handleUpdateStatus = async (id: string, isApproved: boolean) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showToast.success(`Review ${isApproved ? 'approved' : 'rejected'} successfully`);
  ...
  const handleUpdateShowInHome = async (id: string, showInHome: boolean) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showToast.success(`Review ${showInHome ? 'featured on home' : 'removed from home'} successfully`);
        fetchReviews(); // Refresh list
      } else {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-2">
            <MessageSquare size={24} className="text-blue-500" /> Reviews & Feedback
          </h1>
          <p className="text-gray-500 text-sm mt-1">Moderate product reviews submitted by customers.</p>
        </div>
            ...
            <table className="w-full text-left border-collapse min-w-[1000px] text-sm">
              <thead>
                <tr className="bg-gray-50/80 dark:bg-gray-800/50">
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">Product</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">Reviewer</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">Rating</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800 w-1/3">Review</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800 text-right">Actions</th>
                </tr>
              </thead>
                ...
                {reviews.map(review => {
                  ...
                  return (
                    <tr key={review.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-1 flex shrink-0 items-center justify-center">
                            <img src={resolveImageUrl(review.product?.image) || '/placeholder.png'} className="max-w-full max-h-full object-contain" />
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <p className="font-bold text-gray-900 dark:text-white">{review.reviewer || review.user?.name || "Customer"}</p>
                        <p className="text-xs text-gray-500">{review.reviewerEmail || review.user?.email}</p>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map(star => (
                            <Star 
                              key={star} 
                              size={14} 
                              className={star <= review.rating ? "fill-orange-400 text-orange-400" : "fill-gray-200 text-gray-200 dark:fill-gray-700 dark:text-gray-700"} 
                            />
                          ))}
                        </div>
                        <div className="flex flex-col gap-1 mt-2">
                          {review.isApproved ? (
                            <span className="inline-block px-2 py-0.5 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 rounded text-[10px] font-black uppercase tracking-widest border border-green-200 dark:border-green-800 self-start">Approved</span>
                          ) : (
                            <span className="inline-block px-2 py-0.5 bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 rounded text-[10px] font-black uppercase tracking-widest border border-orange-200 dark:border-orange-800 self-start">Pending</span>
                          )}
                          {review.showInHome && (
                            <span className="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded text-[10px] font-black uppercase tracking-widest border border-blue-200 dark:border-blue-800 self-start">Featured (Home)</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        {review.content ? (
                          <p className="text-sm text-gray-600 dark:text-gray-300 italic">"{review.content}"</p>
                        ) : (
                        ...
                        {images.length > 0 && (
                          <div className="flex gap-2 mt-3">
                            {images.map((img, i) => (
                              <a href={resolveImageUrl(img)} target="_blank" rel="noreferrer" key={i}>
                        ...
                        <div className="flex justify-end gap-2">
                          {!review.isApproved ? (
                            <button
                              ...
                              title="Approve"
                            >
                              {actionLoading === review.id ? <Loader2 size={18} className="animate-spin" /> : <Check size={18} />}
                            </button>
                          ) : (
                            <button
                            ...
                            >
                              {actionLoading === review.id ? <Loader2 size={18} className="animate-spin" /> : <X size={18} />}
                            </button>
                          )}
                          <button
                            onClick={() => handleUpdateShowInHome(review.id, !review.showInHome)}
                            disabled={actionLoading === `home-${review.id}` || !review.isApproved}
                            ...
                            title={review.showInHome ? "Remove from Home Page" : "Feature on Home Page"}
                          >
                            {actionLoading === `home-${review.id}` ? <Loader2 size={18} className="animate-spin" /> : <Star size={18} className={review.showInHome ? "fill-current" : ""} />}
                          </button>
                          <button
