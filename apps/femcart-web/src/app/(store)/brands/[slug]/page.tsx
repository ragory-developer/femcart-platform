...
async function getBrand(slug: string) {
  try {
    const res = await fetchWithTimeout(`${API_URL}/api/brands/${slug}`, { next: { revalidate: 3600 } });
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: seo?.description || `Explore high-quality products from ${name}.`,
...
async function getBrandProducts(slug: string, searchParams: any) {
  try {
    ...
    const res = await fetchWithTimeout(`${API_URL}/api/products?${query.toString()}`, { next: { revalidate: 60 } });
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12">
          ...
          <div className="flex items-center gap-3 mb-2">
             ...
             <span className="text-xs font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.2em]">Brand Collection</span>
          </div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-3 font-medium max-w-2xl">
             Explore high-quality products from {brandName}. We bring you the best from our trusted partners.
          </p>
        </div>
