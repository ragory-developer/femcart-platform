...
import { ArrowLeft, ArrowRight, ChevronRight, LayoutGrid, Loader2, Star, Tag } from "lucide-react";
    ...
    <div className="bg-gray-50 dark:bg-gray-950 min-h-[100dvh]">
      ...
      <div className="relative bg-white dark:bg-gray-950 border-b border-gray-100 dark:border-gray-900 pb-12 pt-8 overflow-hidden">
        ...
        <div className="container relative mx-auto px-4">
          <Link href="/" className="inline-flex items-center text-[13px] font-bold font-body text-gray-400 hover:text-[var(--color-olive)] dark:hover:text-[var(--color-lime)] transition-colors mb-10 group">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-5">
                ...
                <span className="text-[var(--color-olive)] dark:text-[var(--color-lime)] font-black font-display uppercase tracking-[0.25em] text-[11px]">Everything you need</span>
              </div>
              <h1 className="text-5xl md:text-7xl font-black font-display text-gray-900 dark:text-white tracking-tighter uppercase leading-[0.9]">
                Explore Our <span className="text-[var(--color-lime)] italic block mt-2">Market</span>
              </h1>
              <p className="text-gray-500 dark:text-gray-400 mt-6 max-w-xl text-[15px] leading-relaxed font-medium font-body">
                Browse our premium selection of fresh groceries, organic produce, household essentials, and trusted premium brands.
              </p>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-md dark:bg-gray-900/80 p-5 rounded-[1.5rem] shadow-[0_8px_30px_rgba(0,0,0,0.04)] dark:shadow-none border border-gray-100 dark:border-gray-800 flex flex-col gap-4">
               <div className="flex items-center gap-5">
                 <div className="text-right">
                    <p className="text-xs font-black font-display text-gray-400 uppercase tracking-widest">Categories</p>
                    <p className="text-xl font-black font-display text-[var(--color-olive)] dark:text-[var(--color-lime)] line-none">{categories.length}</p>
                 </div>
                 <div className="h-8 w-px bg-gray-100 dark:bg-gray-700"></div>
                 <div className="text-left">
                    <p className="text-xs font-black font-display text-gray-400 uppercase tracking-widest">Brands</p>
                    <p className="text-xl font-black font-display text-[#13A048] dark:text-[#13A048] line-none">{brands.length}</p>
                 </div>
               </div>
            </div>
          </div>
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="mb-10 flex items-center gap-4">
           ...
           <h2 className="text-4xl md:text-5xl font-black font-display text-gray-900 dark:text-white uppercase tracking-tight">Categories</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 mb-20">
          {categories.map((category, idx) => (
            ...
            >
              ...
              <Link href={`/categories/${category.slug}`} className="absolute inset-0 z-20" aria-label={category.name}></Link>
              
              <img 
                src={category.image || DEFAULT_CATEGORY_IMAGES[idx % DEFAULT_CATEGORY_IMAGES.length]} 
                alt={category.name}
              ...
              <div className="absolute top-3 left-3 right-3 md:top-6 md:left-6 md:right-6 flex justify-between items-start">
                 <div className="bg-white/90 dark:bg-black/60 backdrop-blur-md text-gray-900 dark:text-gray-100 text-[9px] md:text-[10px] font-bold px-2 py-1 md:px-4 md:py-2 rounded-full uppercase tracking-widest border border-white/50 dark:border-white/10 shadow-sm">
                    {category._count?.products || 0} <span className="hidden md:inline">Products</span><span className="md:hidden">Items</span>
                 </div>
        ...
        <div className="mb-10 flex items-center gap-4">
           ...
           <h2 className="text-4xl md:text-5xl font-black font-display text-gray-900 dark:text-white uppercase tracking-tight">Premium Brands</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 mb-24">
          {brands.map((brand, idx) => (
            ...
            >
              {/* Overlay Link for entire card */}
              <Link href={`/brands/${brand.slug}`} className="absolute inset-0 z-20" aria-label={brand.name}></Link>
              
              <img 
                src={brand.logo || DEFAULT_BRAND_IMAGES[idx % DEFAULT_BRAND_IMAGES.length]} 
                alt={brand.name}
                className="absolute inset-0 w-full h-full object-cover transition-transform duration-[1.5s] group-hover:scale-110 ease-out"
                loading="lazy"
              />
              ...
              <div className="absolute top-3 left-3 right-3 md:top-6 md:left-6 md:right-6 flex justify-between items-start">
                 <div className="bg-white/90 dark:bg-black/60 backdrop-blur-md text-gray-900 dark:text-gray-100 text-[9px] md:text-[10px] font-bold font-body px-2 py-1 md:px-4 md:py-2 rounded-full uppercase tracking-widest border border-white/50 dark:border-white/10 shadow-sm">
                    {brand._count?.products || 0} <span className="hidden md:inline">Products</span><span className="md:hidden">Items</span>
                 </div>
              </div>
        ...
        <div className="mt-24">
           <div className="flex items-center gap-4 mb-12">
              ...
              <h3 className="text-xs font-black font-display text-gray-400 uppercase tracking-[0.5em] text-center">Directory A-Z</h3>
              <div className="h-px flex-1 bg-gray-200 dark:bg-gray-800"></div>
           </div>
           
           <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-5">
              {/* Mixed Directory of Categories and Brands */}
              {[...categories.map(c => ({...c, type: 'category'})), ...brands.map(b => ({...b, type: 'brand'}))]
                 ...
                 >
                    ...
                    <div className="relative z-10 w-12 h-12 bg-gray-50 dark:bg-gray-800 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 group-hover:bg-[#13A048]/10 transition-all duration-500 overflow-hidden border border-gray-100 dark:border-gray-700">
                       {item.type === 'brand' && item.logo ? (
                         ...
                         <LayoutGrid size={20} className="text-gray-400 group-hover:text-[var(--color-olive)] dark:group-hover:text-[var(--color-lime)] transition-colors duration-300" />
                       ) : (
                         <Tag size={20} className="text-gray-400 group-hover:text-[var(--color-olive)] dark:group-hover:text-[var(--color-lime)] transition-colors duration-300" />
                       )}
                    </div>
                    ...
                    <span className="relative z-10 text-[9px] font-bold font-body text-gray-400 block mt-1.5 uppercase tracking-widest transition-colors group-hover:text-[#13A048]">{item._count?.products || 0} ITEMS</span>
                 </Link>
              ))}
           </div>
        </div>
