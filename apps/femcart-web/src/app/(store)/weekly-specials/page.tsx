...
async function getProducts(searchParams: any) {
  try {
    ...
    // Weekly Specials use /api/products/weekly-sale endpoint now
    ...
    const res = await fetch(`${API_URL}/api/products/weekly-sale?${query.toString()}`, {
      cache: 'no-store'
    });
...
export default async function WeeklySpecialsPage({ 
  ...
  const fetchUrl = new URL(`${API_URL}/api/products/weekly-sale`);
  ...
  return (
    <div className="bg-[#fcfaf8] dark:bg-[#0a0a0a] min-h-[100dvh] py-16">
      <div className="container mx-auto px-6 max-w-[1600px]">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-20 border-b border-[#0F3A44]/10 dark:border-gray-900 pb-12">
          <div>
            <Link href="/" className="inline-flex items-center text-[10px] font-black text-[#13A048] hover:text-[#0F3A44] transition-colors mb-6 uppercase tracking-[0.3em]">
              <ArrowLeft size={14} className="mr-2" /> Back to Home
            </Link>
            <h1 className="text-5xl lg:text-[5rem] leading-[1] font-black tracking-tighter uppercase text-[#0F3A44] dark:text-white">
              Weekly Specials
            </h1>
            <p className="text-gray-600 dark:text-gray-400 mt-6 font-medium max-w-xl text-lg leading-relaxed">
              Discover our hand-picked promotions and limited-time deals on your favorite grocery and household items!
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-14">
          {/* Sidebar - Desktop */}
          <aside className="hidden lg:block sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto w-72 shrink-0 lg:border-r lg:border-[#0F3A44]/10 lg:dark:border-gray-800/60 lg:pr-8">
          ...
          <div className="flex-1">
            ...
            {/* Toolbar */}
            <Suspense fallback={<div className="h-20 w-full animate-pulse bg-gray-100 dark:bg-gray-800 rounded-2xl mb-8" />}>
