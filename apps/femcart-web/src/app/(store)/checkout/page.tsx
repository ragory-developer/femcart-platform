...
function CheckoutContent() {
  ...
  const { items, getCartTotal, clearCart } = useCartStore(useShallow(state => ({
    items: state.items,
    getCartTotal: state.getCartTotal,
    clearCart: state.clearCart
  })));
  
  const { user, isAuthenticated, fetchUser } = useAuthStore(useShallow(state => ({
    user: state.user,
    isAuthenticated: state.isAuthenticated,
    fetchUser: state.fetchUser
  })));
  
  const { settings, fetchSettings, loading: settingsLoading } = useSettingsStore(useShallow(state => ({
  ...
  const [orderTotals, setOrderTotals] = useState({ subtotal: getCartTotal(), deliveryFee: 0, discount: 0, total: getCartTotal() });
  ...
  const submitOrder = async () => {
    ...
    if (!activeUser) return showToast.error("Please verify your account first.");
    if (!selectedAddress) return showToast.error("Please provide a delivery address.");
    
    if (paymentMethod === "COD") {
      if (!selectedAddress.recipientName) return showToast.error("Recipient Name is missing from the address.");
      if (!selectedAddress.recipientPhone) return showToast.error("Recipient Phone is missing from the address.");
      if (!selectedAddress.address) return showToast.error("Street Address is missing.");
      if (!selectedAddress.stateId) return showToast.error("State is missing from the address.");
      if (!selectedAddress.cityId) return showToast.error("City is missing from the address.");
      if (!selectedAddress.areaId) return showToast.error("ZIP / Area is missing from the address.");
    }

    if (!guestEmail || !guestEmail.includes('@')) {
      return showToast.error("A valid email address is required for your receipt.");
    }
    ...
    try {
      ...
      if (data.success) {
        if (paymentMethod === 'STRIPE' || paymentMethod === 'BKASH' || paymentMethod === 'CARD' || paymentMethod === 'NAGAD') {
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 py-6">
        <div className="container mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center flex-wrap justify-center gap-1 sm:gap-2 text-xs sm:text-sm font-bold text-gray-500 dark:text-gray-400">
            ...
            <span className={activeUser ? "text-blue-600 dark:text-blue-400" : ""}>Shipping</span>
            <ChevronRight size={14} className="sm:w-4 sm:h-4" />
            <span className={activeUser ? "text-blue-600 dark:text-blue-400" : ""}>Payment</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex-1 w-full space-y-6">
            ...
            {!activeUser && (
              !guestMode ? (
                ...
                <EmailVerification onVerified={(u) => { setVerifiedGuestUser(u); }} onCancel={() => setGuestMode(false)} />
              )
            )}

            {activeUser && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <AddressForm user={activeUser} onAddressSelect={setSelectedAddress} />
                
                <PaymentSelector selected={paymentMethod} onSelect={setPaymentMethod} settings={settings} />
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl lg:rounded-3xl p-5 lg:p-8 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                {items.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="flex gap-4 items-start">
                      <Link href={productUrl} className="w-16 h-16 rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 overflow-hidden flex-shrink-0 relative cursor-pointer hover:opacity-80 transition-opacity">
                        {item.image ? (
              ...
              <div className="space-y-4 pt-6 border-t border-gray-100 dark:border-gray-700">
                ...
                <div className="pt-6 mt-2 border-t border-gray-100 dark:border-gray-700 flex justify-between items-end">
                  <div>
                    <span className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tighter italic block leading-none">Total</span>
                ...
                <div className="mt-8">
                  {paymentMethod === 'PAYPAL' ? (
                    <div className="p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/50">
                      <PayPalScriptProvider options={{ clientId: settings.paypal_client_id || process.env.NEXT_PUBLIC_PAYPAL_CLIENT_ID || "test", currency: "USD" }}>
                        <PayPalIntegration 
                    ...
                    >
                      {loading ? <Loader2 className="animate-spin" size={24} /> : (
                        <>
                          <ShieldCheck size={22} />
                          <span>{['STRIPE', 'CARD', 'NAGAD', 'BKASH'].includes(paymentMethod) ? 'Pay Securely' : 'Place Order Now'}</span>
                        </>
                      )}
                    </button>
                  )}
