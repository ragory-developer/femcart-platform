...
class EnterpriseLogger {
  ...
  private getDeviceContext() {
    if (typeof window === 'undefined') {
      return { user_agent: 'server', url: 'server', screen_resolution: 'server' };
    }
    return {
  ...
  private getUserId() {
    ...
    try {
      const userStr = localStorage.getItem('femcart_user') || localStorage.getItem('user');
      if (userStr) {
  ...
  private enqueueLog(level: LogLevel, message: string, options?: LogOptions) {
    ...
    if (this.logQueue.length >= this.maxQueueSize || level === 'error' || level === 'fatal') {
      this.flushLogs();
    }
  }
  ...
  public initializeGlobalErrorHandling() {
    ...
    window.addEventListener('error', (event) => {
      this.fatal('Uncaught Error', { error: event.error, data: { message: event.message, filename: event.filename, lineno: event.lineno } });
    });
  ...
  private printToConsole(level: LogLevel, message: string, options?: LogOptions) {
    ...
    const loggerFn = level === 'error' || level === 'fatal' ? console.error : level === 'warn' ? console.warn : console.info;
  ...
  public trackPerformance(metricName: string, value: number, tags?: Record<string, string>) {
    // Specialized format for performance metrics
    this.enqueueLog('info', `Performance: ${metricName}`, { data: { metric: metricName, value, tags }, context: 'Performance' });
  }
}
