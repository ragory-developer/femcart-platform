...
import { Apple, Baby, ShieldAlert, Sparkles, Dog, HeartPulse, ShoppingBag, Utensils, PenTool, Smartphone, Beef, Carrot, Milk, Package } from "lucide-react";
...
export function getProductImage(product: any): string {
  ...
  const imgUrl = (imagesArray.length > 0 ? imagesArray[0] : null) || product.featuredImage;
...
export function getCategoryIcon(title: string) {
  ...
  if (t.includes("meat") || t.includes("beef") || t.includes("chicken")) return Beef;
  if (t.includes("fruit") || t.includes("veg") || t.includes("produce")) return Carrot;
  ...
  if (t.includes("gadget") || t.includes("tech") || t.includes("phone")) return Smartphone;
