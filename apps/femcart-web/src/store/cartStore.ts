    ...
    (set, get) => {
      ...
      syncLocalCartToBackend: async () => {
        ...
        syncTimeout = setTimeout(async () => {
          ...
          try {
            const items = get().items.map(i => ({ productId: i.productId || i.id, variantId: i.variantId, quantity: i.quantity }));
        ...
        try {
          ...
          if (data.success && data.data && data.data.items) {
            ...
            const backendItems = data.data.items.map((i: any) => ({
              ...
              name: i.variantId && i.variant?.attributes ? `${i.product?.name} - ${i.variant.attributes.map((a:any) => a.value).join(' / ')}` : i.product?.name || "Unknown",
              price: i.variantId ? (i.variant?.specialPrice || i.variant?.price || 0) : (i.product?.specialPrice || i.product?.price || 0),
              image: i.variantId ? (i.variant?.image || i.product?.image || "") : (i.product?.image || ""),
              quantity: i.quantity,
              slug: i.product?.slug || ""
            }));
