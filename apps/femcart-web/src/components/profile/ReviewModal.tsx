...
export default function ReviewModal({ orderId, items, onClose, onSuccess }: ReviewModalProps) {
  ...
  const [productReviews, setProductReviews] = useState(
    items.map(item => ({
      ...
      name: item.variant && item.variant.attributes?.length 
  ...
  const handleMediaSelect = (media: any, url: string | string[]) => {
    ...
    setProductReviews(prev => {
      const clone = [...prev];
      clone[activeItemIdx].images = [...clone[activeItemIdx].images, ...incoming];
      return clone;
    });
  };
  ...
  const removeImage = (itemIdx: number, imgIdx: number) => {
    setProductReviews(prev => {
      const clone = [...prev];
      clone[itemIdx].images = clone[itemIdx].images.filter((_, i) => i !== imgIdx);
      return clone;
    });
  };
          ...
          <AnimatePresence mode="wait">
            {step === 0 && (
              ...
              >
                ...
                <h3 className="text-2xl font-black text-gray-900 dark:text-white mb-2 tracking-tight">How was your delivery?</h3>
                <p className="text-gray-500 font-medium mb-8">Please rate your experience with the courier and packaging.</p>
                ...
                <div className="w-full text-left">
                  <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Additional Comments</label>
                  <textarea
                    rows={4}
              ...
              >
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-16 h-16 rounded-2xl border border-gray-100 dark:border-gray-800 p-1 shrink-0">
                    <img src={resolveImageUrl(productReviews[step - 1].image) || '/placeholder.png'} className="w-full h-full object-contain" />
                  </div>
                  <div>
                ...
                <div className="w-full text-left mb-6">
                  <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Your Review</label>
                  <textarea
                    rows={3}
                ...
                <div className="w-full text-left">
                  <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Photo Evidence</label>
                  
                  <div className="flex flex-wrap gap-2">
                    {productReviews[step - 1].images.map((img, idx) => (
                      <div key={idx} className="relative w-16 h-16 rounded-xl border border-gray-200 dark:border-gray-700 group overflow-hidden">
        ...
        <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center">
          <div className="flex gap-1">
            {Array.from({ length: totalSteps }).map((_, i) => (
              <div key={i} className={`h-1.5 rounded-full transition-all ${i === step ? 'w-8 bg-blue-600' : i < step ? 'w-2 bg-blue-300' : 'w-2 bg-gray-200 dark:bg-gray-700'}`} />
            ))}
          </div>
          ...
          >
            {loading ? <Loader2 size={16} className="animate-spin" /> : (
              <>{step === totalSteps - 1 ? 'Submit' : 'Next'} {step !== totalSteps - 1 && <ChevronRight size={16} />}</>
            )}
          </button>
        </div>
      </motion.div>
