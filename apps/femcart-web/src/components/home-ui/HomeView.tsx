...
export default function HomeView({ categories, globalSettings, products = [] }: { categories: any, globalSettings: any, products?: any[] }) {
  let layout: any = [];
  try {
  ...
  if (!layout || layout.length === 0) {
    layout = [
      ...
      { id: "feat-1", type: "FeaturedProducts", props: { productSource: "FEATURED", limit: 10 } },
      { id: "size-1", type: "SizeBanner", props: {} },
      { id: "best-1", type: "BestSellers", props: { productSource: "BEST_SELLERS", limit: 10 } },
      { id: "lim-1", type: "LimitedOffers", props: {} },
      { id: "new-1", type: "NewArrivals", props: { productSource: "NEW_ARRIVALS", limit: 10 } },
    ...
    <div className="flex flex-col w-full">
      {layout.map((section: any) => {
        ...
        if (section.props?.productSource === "FEATURED") {
          ...
          sectionProducts = products.filter((p: any) => p.specialPrice && p.specialPrice < p.price);
        } else if (section.props?.productSource === "BEST_SELLERS") {
          sectionProducts = [...products].sort((a: any, b: any) => (b.averageRating || 0) - (a.averageRating || 0));
        } else if (section.props?.productSource === "WEEKLY_SALE") {
          sectionProducts = products.filter((p: any) => p.specialPrice && p.specialPrice < p.price).slice(0, section.props.limit || 10);
        } else if (section.props?.productSource === "NEW_ARRIVALS") {
          sectionProducts = [...products].sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        }
        ...
        return (
          <ClientInViewSection key={section.id}>
             <Component {...section.props} categories={categories} products={sectionProducts} />
          </ClientInViewSection>
        );
      })}
    </div>
