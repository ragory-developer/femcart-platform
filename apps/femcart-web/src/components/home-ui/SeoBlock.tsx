...
export default function SeoBlock({
  ...
  return (
    <section className="max-w-[1440px] mx-auto px-4 md:px-6 mb-4 md:mb-16 max-w-4xl text-center">
      <h2 className="text-[22px] md:text-[28px] font-serif mb-4 md:mb-6">{title}</h2>
      <p className="text-[13px] md:text-[14px] text-text-amber-700 leading-snug md:leading-relaxed mb-4 md:mb-6">
        {description}
      </p>
      {buttonText && (
        <Link href={buttonLink || "#"} className="inline-flex items-center justify-center h-12 px-6 rounded-full font-sans font-semibold text-[15px] tracking-[0.3px] transition-all duration-150 cursor-pointer bg-white text-pink-500 border-[1.5px] border-pink-500 border-none bg-orange-200/40 hover:bg-orange-200/60 text-text-pink-500 px-8">
          {buttonText}
        </Link>
      )}
    </section>
