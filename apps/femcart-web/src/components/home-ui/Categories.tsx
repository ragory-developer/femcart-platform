          ...
          <SwiperSlide key={c.id}>
            ...
            >
              <div className="relative aspect-[4/5] rounded-[24px] overflow-hidden mb-4 shadow-sm group-hover:shadow-md transition-all duration-300 group-hover:ring-4 group-hover:ring-pink-500/20 group-hover:ring-offset-2 ring-offset-white">
                <img src={c.image || 'https://images.unsplash.com/photo-1517841905240-472988babdf9?q=80&w=400&auto=format&fit=crop'} alt={c.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-60 group-hover:opacity-80 transition-opacity"></div>
              </div>
              <h3 className="text-center font-serif text-[15px] md:text-[18px] lg:text-[20px] text-text-pink-500 group-hover:text-pink-500 transition-colors">{c.name}</h3>
            </motion.a>
          </SwiperSlide>
        ))}
      </Swiper>
