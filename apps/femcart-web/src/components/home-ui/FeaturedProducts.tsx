    ...
    <section className="max-w-[1440px] mx-auto px-4 md:px-6 mb-4 md:mb-16">
      ...
      <div className="w-full bg-pink-500/10 rounded-[24px] overflow-hidden mb-12 flex flex-col md:flex-row items-center border border-pink-500/20">
        <div className="md:w-1/2 p-4 md:p-8 text-center md:text-left">
          <span className="text-pink-500 font-semibold text-[13px] tracking-wider uppercase mb-2 block">{bannerBadge}</span>
          <h3 className="font-serif text-3xl md:text-4xl lg:text-5xl mb-2">{bannerTitle}</h3>
          <p className="text-amber-700 text-[13px] md:text-[15px] leading-snug md:leading-normal mb-4 md:mb-6">{bannerDesc}</p>
          {bannerButtonText && (
            <button className="inline-flex items-center justify-center h-10 px-6 rounded-full font-sans font-semibold text-[14px] tracking-[0.3px] transition-all duration-150 cursor-pointer bg-pink-500 text-white hover:bg-pink-600 active:scale-[0.98]">{bannerButtonText}</button>
          )}
        </div>
        <div className="md:w-1/2 h-64 md:h-[280px] self-stretch">
          <img src={bannerImage} alt={bannerTitle} className="w-full h-full object-cover" />
        </div>
      </div>
