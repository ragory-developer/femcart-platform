        ...
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 w-full flex justify-between items-center">
          ...
          <div className="flex items-center justify-start flex-shrink-0">
            ...
            <a href="/" className="relative flex flex-col items-center justify-center font-bold text-[26px] text-[#E32857]" style={{ letterSpacing: '-0.5px' }}>
              {settings?.store_logo && settings.store_logo !== "null" && settings.store_logo !== "undefined" && !logoError ? (
                <img 
                ...
                <>
                  ...
                  <span className="leading-none mt-1">{settings?.store_name || "Femcart"}</span>
                </>
              )}
            </a>
          </div>
          ...
          <ul className="hidden lg:flex flex-1 justify-center gap-8 xl:gap-10 list-none items-center text-gray-800 text-[14px] font-medium whitespace-nowrap px-4">
            {topNavbarItems?.map((item: any) => {
              ...
              const isActive = pathname === item.url || pathname === getFilterUrl(item.url || "#");
              
              if (hasDropdown) {
                return (
                  <li key={item.id} className="relative group flex items-center gap-1 cursor-pointer transition-colors py-4">
                    <span className={`hover:text-[#E32857] transition-colors ${isActive ? 'text-[#E32857]' : ''}`}>{item.title}</span> 
                    ...
                    <div className="absolute top-full left-0 hidden group-hover:block z-50 -mt-2 pt-2">
                      <ul className="bg-white border border-gray-100 shadow-xl py-3 min-w-[220px] flex flex-col font-normal text-[14.5px] text-gray-800">
                        {item.children.map((child: any) => (
                          <li key={child.id}>
                            <Link href={getFilterUrl(child.url || "#")} className="block px-6 py-2.5 hover:bg-rose-50 hover:text-[#E32857] transition-colors">
                              {child.title}
                            </Link>
                          </li>
                        ))}
              ...
              return (
                <li key={item.id}>
                  <Link href={getFilterUrl(item.url || "#")} className={`relative transition-colors py-2 block ${isActive ? 'text-[#E32857]' : 'hover:text-[#E32857]'}`}>
                    {item.title}
                    {isActive && <span className="absolute -bottom-[2px] left-0 w-full h-[1.5px] bg-[#E32857]"></span>}
                  </Link>
                </li>
              );
            })}
              ...
              <nav className="flex flex-col p-6 gap-6 overflow-y-auto">
                {topNavbarItems?.map((item: any) => (
                  <React.Fragment key={item.id}>
                    <Link onClick={() => setIsMobileMenuOpen(false)} href={getFilterUrl(item.url || "#")} className="text-[18px] font-medium text-text-pink-500 hover:text-pink-500 transition-colors">
                      {item.title}
                    </Link>
                    {item.children && item.children.length > 0 && item.children.map((child: any) => (
                      <Link onClick={() => setIsMobileMenuOpen(false)} key={child.id} href={getFilterUrl(child.url || "#")} className="text-[16px] font-normal text-text-amber-700 pl-4 hover:text-pink-500 transition-colors">
                        {child.title}
                      </Link>
                    ))}
                  </React.Fragment>
      ...
      <footer className="bg-white pt-10 md:pt-16 pb-8 border-t border-orange-200 mt-8 md:mt-20">
        <div className="max-w-[1440px] mx-auto px-4 md:px-6 grid grid-cols-2 md:flex md:flex-row gap-y-10 gap-x-4 md:gap-12 justify-between">
          <div className="col-span-2 md:col-auto md:w-1/3">
            <a href="/" className="font-serif font-semibold text-pink-500 text-[32px] inline-block mb-4">
              {settings?.store_logo && settings.store_logo !== "null" && settings.store_logo !== "undefined" && !logoError ? (
                <img 
                  src={settings.store_logo} 
                  alt={settings.store_name || "Femecart"} 
            ...
            <p className="text-text-amber-700 text-[14px] mb-4">{settings?.footer_about_text || "Providing comfortable bras and panties to women across Bangladesh."}</p>
            <p className="text-text-pink-500 text-[14px] font-medium flex items-center gap-2">
              <Phone size={16} className="text-pink-500" strokeWidth={2} /> {settings?.footer_phone || "+880 1812 345678"}
            </p>
            <p className="text-text-pink-500 text-[14px] font-medium flex items-center gap-2 mt-2">
              <Mail size={16} className="text-pink-500" strokeWidth={2} /> {settings?.footer_email || "support@femecart.com"}
            </p>
          </div>
          
          {footerSections?.map((section: any) => (
            <div key={section.id} className="col-span-1 md:col-auto md:w-1/4">
              <h4 className="font-serif text-[16px] md:text-[18px] mb-3 md:mb-4">{section.title}</h4>
        ...
        <div className="max-w-[1440px] mx-auto px-4 md:px-6 border-t border-orange-200 mt-10 md:mt-12 pt-6 flex flex-col items-center justify-center text-text-amber-700 text-[13px] gap-2">
          <div className="flex gap-4 items-center">
            ...
            ].filter(Boolean).join(" • ") || "No payment methods configured"}
          </div>
          <p>&copy; {new Date().getFullYear()} {settings?.store_name || "Femecart"}. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
