...
export default function SearchableDropdown({
  ...
  const text = size === "sm" ? "text-[clamp(0.875rem,2vw,1rem)]" : "text-[clamp(1rem,2vw,1.125rem)]";

  return (
    <div ref={containerRef} className={`relative ${className} transition-all ${open ? 'z-50' : 'z-10'}`}>
      {/* Trigger button */}
      <button
        type="button"
        disabled={disabled}
