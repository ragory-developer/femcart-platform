...
function ProductCard({
  ...
  product: any;
  prefetch?: boolean;
  variant?: "classic" | "sleek" | "minimal" | "festive" | "bordered" | "neumorphic" | "horizontal" | "elegant";
  ...
  const variant = mounted ? (propVariant ?? (settings.productCardVariant as any) ?? "classic") : (propVariant ?? "classic");
  const radius = mounted ? (propRadius ?? (settings.productCardRadius as any) ?? "sm") : (propRadius ?? "sm");
  const showBadge = mounted ? (propShowBadge ?? settings.productCardShowBadge ?? true) : (propShowBadge ?? true);
  const showRating = mounted ? (propShowRating ?? settings.productCardShowRating ?? true) : (propShowRating ?? true);
  const showAddToCart = mounted ? (propShowAddToCart ?? settings.productCardShowAddToCart ?? true) : (propShowAddToCart ?? true);
  const badgeStyle = mounted ? (propBadgeStyle ?? (settings.productCardBadgeStyle as any) ?? "pill") : (propBadgeStyle ?? "pill");
  ...
  const imageRadiusClass = imageRadiusClasses[radius as string] || "rounded-t-sm";
  ...
  const renderRatingStars = () => {
    ...
    const ratingValue = product.averageRating != null ? Number(product.averageRating) : 0.0;
    return (
  ...
  return (
    <div className="group relative h-full flex flex-col" onMouseEnter={handleMouseEnter}>
      {/* Absolute overlay link covering the entire card */}
      <Link href={productUrl} prefetch={prefetch} aria-label={product.name} className="absolute inset-0 z-[1] rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500" />
      
      <div className={`transition-[transform,border-color,box-shadow] duration-500 will-change-transform overflow-hidden flex ${variant === "horizontal" ? "flex-row items-center h-auto min-h-[140px]" : "flex-col h-full"} relative ${cardRadiusClass} ${variant === "elegant" ? "bg-gradient-to-b from-emerald-50/50 to-white dark:from-emerald-950/20 dark:to-gray-900 border border-emerald-100 dark:border-emerald-900/30 shadow-sm" : "bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm"} hover:border-emerald-500/30 hover:shadow-[0_8px_30px_rgba(16,185,129,0.12)] hover:-translate-y-1`}>
        {/* Shimmer/glass hover effect */}
        {variant !== "minimal" && (
          <div className={`absolute inset-0 bg-gradient-to-tr opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none ${variant === "festive"
        ...
        {/* Image area */}
        <div
          className={`relative block overflow-hidden ${variant === "horizontal" ? "w-[120px] shrink-0" : ""} ${variant === "classic"
        ...
        >
          ...
          {((product.productType === 'VARIABLE' && (!product.variants || product.variants.filter((v: any) => v.enabled !== false).every((v: any) => v.stock === 0))) || (product.productType !== 'VARIABLE' && (product.stock === 0 || product.inStock === false))) && (
          ...
          <div className={`aspect-[3/4] sm:aspect-square relative overflow-hidden ${variant === "horizontal" ? "w-[120px]" : "w-full"} ${variant === "minimal" ? "bg-gray-50 dark:bg-gray-800 rounded-sm" : "bg-white dark:bg-transparent"}`}>
            <Image
              src={imageSrc}
            ...
            >
              <Heart size={14} className={`sm:w-4 sm:h-4 ${isWishlisted ? "fill-rose-500" : ""}`} />
            </button>
          </div>
        </div>

        {/* Add to Cart floating icon (Classic/Festive/Elegant variant) */}
        {showAddToCart && (variant === "classic" || variant === "festive" || variant === "elegant") && (
          <button
            type="button"
        ...
        {/* Body content */}
        <div className={`flex flex-col flex-1 gap-0.5 sm:gap-1.5 ${variant === "minimal" ? "pt-1.5 sm:pt-3 px-1.5 sm:px-2 pb-1.5 sm:pb-2" : variant === "horizontal" ? "p-1.5 sm:p-3 pl-1" : "p-1.5 sm:p-3"}`}>
          {/* Category */}
          {(product.categories?.[0]?.name || product.category?.name) && (
            <span className={`text-[8px] sm:text-[clamp(10px,1.5vw,11px)] font-bold uppercase tracking-[0.05em] truncate ${variant === "festive" ? "text-amber-600 dark:text-amber-400" : "text-emerald-600/80"
              }`}>
              {product.categories?.[0]?.name ?? product.category?.name}
            </span>
          )}
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap mt-0.5 sm:mt-1">
            <span className={`font-extrabold ${variant === "festive"
