...
import { CheckCircle2, ChevronDown, Loader2, MapPin, Pencil, Plus, Trash2, X } from "lucide-react";
...
export default function AddressForm({ onAddressSelect, user }: AddressFormProps) {
  ...
  const getToken = () => localStorage.getItem('femcart_access_token') || localStorage.getItem('token') || '';
  ...
  const fetchCities = useCallback(async (stateId: string, resetChildren = true) => {
  ...
  const fetchAreas = useCallback(async (cityId: string, resetChildren = true) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Strict Validation
    if (!formData.recipientName.trim()) return showToast.error("Recipient Name is required.");
    if (!formData.recipientPhone.trim()) return showToast.error("Recipient Mobile is required.");
    ...
    if (!formData.address.trim()) return showToast.error("Street Address is required.");
  ...
  const inputClass = "w-full px-4 py-3 min-h-[44px] rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:border-blue-500 text-[16px] sm:text-sm transition-colors";
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl lg:rounded-xl p-5 lg:p-8 shadow-sm border border-gray-100 dark:border-gray-700">
      <div className="flex items-center justify-between mb-4 lg:mb-6">
        ...
        {!showForm && !user?.isGuest && (
          <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 text-sm lg:text-base font-bold text-blue-600 hover:text-blue-700 transition-colors min-h-[44px]">
            <Plus size={16} /> Add New
          </button>
        )}
        {showForm && (editingId || addresses.length > 0) && (
          <button onClick={resetForm} className="flex items-center gap-1.5 text-sm lg:text-base font-medium text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors min-h-[44px]">
            <X size={16} /> Cancel
          </button>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-2">
          {addresses.map(addr => (
            ...
            >
              ...
              {/* Edit / Delete actions */}
              <div className="flex gap-2 mt-3 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="space-y-4 lg:space-y-6 mt-2">
            ...
            <div>
              ...
              {loadingCities ? (
                ...
                <SearchableDropdown
                  ...
                  placeholder={!formData.stateId ? "Select state first" : "Select city..."}
            ...
            <div>
              ...
              {loadingAreas ? (
                ...
                <SearchableDropdown
                  ...
                  placeholder={!formData.cityId ? "Select city first" : "Select ZIP / area..."}
          ...
          <div className="flex gap-3 pt-1">
            <button type="submit" disabled={saving} className="flex-1 py-3 lg:py-4 min-h-[48px] bg-gray-900 hover:bg-black dark:bg-white dark:hover:bg-gray-100 dark:text-black text-white rounded-xl font-bold text-sm lg:text-base transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
