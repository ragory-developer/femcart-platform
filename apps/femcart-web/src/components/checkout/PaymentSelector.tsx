...
export default function PaymentSelector({ selected, onSelect, settings = {} }: PaymentSelectorProps) {
  const methods = [
    ...
    { id: 'STRIPE', name: 'Stripe (Card)', desc: 'Pay securely via Stripe', icon: CreditCard, color: 'text-indigo-600', bg: 'bg-indigo-50 dark:bg-indigo-900/20', border: 'border-indigo-500' },
    { id: 'PAYPAL', name: 'PayPal', desc: 'Pay securely via PayPal', icon: Wallet, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-900/20', border: 'border-blue-500' },
  ...
  const availableMethods = methods.filter(m => settings[`payment_enable_${m.id.toLowerCase()}`] !== 'false');

  // If currently selected method is not available, auto-select the first available one
  useEffect(() => {
    if (availableMethods.length > 0 && (!selected || !availableMethods.find(m => m.id === selected))) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
        {availableMethods.map((m) => {
          ...
          return (
            ...
            >
              <div className={`w-10 h-10 lg:w-12 lg:h-12 shrink-0 rounded-xl flex items-center justify-center ${isSelected ? 'bg-white dark:bg-gray-800 shadow-sm' : m.bg}`}>
                <Icon className={`${m.color} w-5 h-5 lg:w-6 lg:h-6`} />
              </div>
              <div>
                <h3 className="font-bold text-gray-900 dark:text-white text-sm lg:text-base">{m.name}</h3>
                <p className="text-xs lg:text-sm text-gray-500 dark:text-gray-400 mt-0.5">{m.desc}</p>
              </div>
            </div>
          );
        })}
