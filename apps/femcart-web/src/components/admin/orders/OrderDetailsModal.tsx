...
import { Clock, MapPin, Package, Settings, X, Printer, CheckCircle2, CircleDashed, Truck, Check, Copy, CheckCheck, RotateCcw, Leaf } from "lucide-react";
...
export function OrderDetailsModal({
  ...
  const [refundMethod, setRefundMethod] = useState<'WALLET' | 'ORIGINAL' | 'MANUAL'>('WALLET');
  ...
  const handleProcessReturn = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/orders/${selectedOrder.id}/return-items`, {
        method: 'POST',
        headers: {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const token = localStorage.getItem('femcart_access_token') || localStorage.getItem('token');
      const res = await fetch(`${API_URL}/api/orders/${selectedOrder.id}/notes`, {
        method: 'POST',
  ...
  const handleUpdateTracking = async () => {
    ...
    try {
      ...
      if (data.success) {
        setSelectedOrder(data.data);
        setOrders(prev => prev.map(o => o.id === selectedOrder.id ? data.data : o));
  ...
  const actualStageIndex = STEPPER_STAGES.findIndex(s => s.value === selectedOrder.status);
  const displayStatus = pendingStatus || selectedOrder.status;
  const currentStageIndex = STEPPER_STAGES.findIndex(s => s.value === displayStatus);
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 print:p-0 print:block print:relative print:z-auto">
        <div className="absolute inset-0 bg-black/60 backdrop-blur-md print:hidden" onClick={() => setSelectedOrder(null)} />
        <div className="relative bg-white dark:bg-gray-900 w-full max-w-[1000px] max-h-[95vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] rounded-xl shadow-2xl border border-white/10 slide-up print:shadow-none print:border-none print:w-full print:max-w-none print:h-auto print:max-h-none print:rounded-none print:overflow-visible flex flex-col">
          ...
          <div className="sticky top-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border-b border-gray-100 dark:border-gray-800 p-6 flex justify-between items-center z-20 print:hidden">
            <div className="flex items-center gap-4">
              <div>
                <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Order #{selectedOrder.id}</h2>
                <p className="text-xs text-gray-500 font-bold uppercase tracking-widest mt-0.5">{new Date(selectedOrder.createdAt).toLocaleString()}</p>
              </div>
              {isCancelled && (
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setSelectedOrder(null)} className="p-2 bg-gray-100 dark:bg-gray-800 hover:bg-rose-100 hover:text-rose-600 dark:hover:bg-rose-900/30 dark:hover:text-rose-400 rounded-full transition-colors ml-2">
                <X size={20} />
              </button>
            </div>
          </div>
                ...
                <div className="flex items-center justify-between relative">
                  ...
                  <div className="absolute left-[5%] right-[5%] top-1/2 h-1 -translate-y-1/2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden z-0">
                    <div 
                      ...
                      style={{ width: currentStageIndex > 0 ? `${(currentStageIndex / (STEPPER_STAGES.length - 1)) * 100}%` : '0%' }}
                    />
                  </div>
                  ...
                  {STEPPER_STAGES.map((stage, idx) => {
                    ...
                    const canClick = !updatingStatus && idx === actualStageIndex + 1;

                    return (
                      <div 
                        key={stage.value} 
                        className={`relative z-10 flex flex-col items-center gap-2 ${canClick ? 'cursor-pointer group' : ''}`}
                        onClick={() => canClick && setPendingStatus(stage.value)}
                      >
                ...
                {pendingStatus && pendingStatus !== selectedOrder.status && (
                  <div className="mt-8 flex justify-end items-center gap-4 animate-in fade-in slide-in-from-top-2">
                    <span className="text-sm font-bold text-gray-500 uppercase tracking-widest">
                      Update status to {STEPPER_STAGES.find(s => s.value === pendingStatus)?.label}?
                    </span>
                    <button 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 print:grid-cols-2">
                  <div className="bg-gradient-to-br from-gray-50 to-white dark:from-gray-800/80 dark:to-gray-800/40 p-6 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm relative group">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      {selectedOrder.user?.id && (
                        <button onClick={() => window.open(`/admin/customers/${selectedOrder.user.id}`, '_blank')} className="text-[10px] font-bold text-emerald-600 hover:text-emerald-700 uppercase tracking-widest print:hidden">
                          View Profile ↗
                        </button>
                      )}
                    </div>
                    <div className="flex gap-4 items-center">
                      <div className="w-14 h-14 rounded-lg bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 flex items-center justify-center font-black text-2xl shadow-inner">
                        {selectedOrder.customerName ? selectedOrder.customerName[0].toUpperCase() : (selectedOrder.user?.name ? selectedOrder.user.name[0].toUpperCase() : 'G')}
                      </div>
                      <div className="flex-1">
                        <p className="text-lg font-black text-gray-900 dark:text-white leading-none">
                          ...
                          {selectedOrder.user?.isGuest && <span className="ml-2 text-[8px] font-black uppercase bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-md align-middle">GUEST</span>}
                        </p>
                        <div className="flex items-center gap-2 mt-2 group/phone">
                          <p className="text-sm text-gray-500 font-bold">{selectedOrder.customerPhone || selectedOrder.user?.phone || "No phone"}</p>
                          <button onClick={() => handleCopy(selectedOrder.customerPhone || selectedOrder.user?.phone || "", "Phone")} className="opacity-0 group-hover/phone:opacity-100 transition-opacity p-1 text-gray-400 hover:text-emerald-500 print:hidden">
                            {copiedField === "Phone" ? <CheckCheck size={14} /> : <Copy size={14} />}
                          </button>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-gray-50 to-white dark:from-gray-800/80 dark:to-gray-800/40 p-6 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm relative group">
                    ...
                    <div className="group/address relative">
                      <p className="text-sm font-bold text-gray-900 dark:text-white leading-snug pr-8">{selectedOrder.deliveryAddress}</p>
                      ...
                      <button 
                        onClick={() => handleCopy(`${selectedOrder.deliveryAddress}, ${[selectedOrder.deliveryArea, selectedOrder.deliveryCity, selectedOrder.deliveryState].filter(Boolean).join(", ")}`, "Address")} 
                      ...
                      >
                        {copiedField === "Address" ? <CheckCheck size={14} /> : <Copy size={14} />}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800/50 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 print:border-none print:shadow-none print:p-0">
                  ...
                  <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar print:max-h-none print:overflow-visible">
                    {selectedOrder.items.map((item, idx) => {
                      ...
                      const availableToReturn = item.quantity - returnedQty - damagedQty;
                      const showReturnBtn = availableToReturn > 0 && ['DELIVERED', 'COMPLETED', 'PARTIALLY_RETURNED'].includes(selectedOrder.status);

                      return (
                        <div key={idx} className="pb-4 border-b border-gray-100 dark:border-gray-700 last:border-0 last:pb-0 print:border-b-2 print:border-gray-200">
                          <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-bold text-gray-900 dark:text-white leading-tight">
                                ...
                                {item.variant && item.variant.attributes?.length > 0 && (
                                  <span className="ml-1 text-xs font-normal text-gray-500">
                                    ({item.variant.attributes.map((a: any) => a.value).join(' / ')})
                                  </span>
                                )}
                              </span>
                            ...
                            <div className="text-right flex flex-col items-end">
                              <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">৳{(item.quantity * item.price).toFixed(2)}</span>
                              {showReturnBtn && (
                                <button 
                                ...
                                >
                                  <RotateCcw size={14} className="group-hover/btn:-rotate-90 transition-transform duration-300" />
                                  Process Return
                                </button>
                              )}
                            </div>
                            ...
                            <div className="mt-3 p-3 bg-rose-50/50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900/30 rounded-xl flex items-center gap-4">
                              {returnedQty > 0 && (
                                <div className="text-[10px] text-rose-700 dark:text-rose-300">
                                  <span className="font-bold text-rose-600 dark:text-rose-400">RESTOCKED:</span> {returnedQty}
                                </div>
                              )}
                              {damagedQty > 0 && (
                                <div className="text-[10px] text-orange-700 dark:text-orange-300">
                                  <span className="font-bold text-orange-600 dark:text-orange-400">DAMAGED:</span> {damagedQty}
                                </div>
                              )}
                  ...
                  <div className="mt-8 pt-6 border-t border-gray-100 dark:border-gray-700 space-y-3 print:border-t-4 print:border-gray-900">
                    ...
                    {((selectedOrder as any).refundAmount || 0) > 0 && (
                      <div className="flex justify-between text-xs text-rose-600 dark:text-rose-400 font-bold">
                        ...
                        <span>-৳{((selectedOrder as any).refundAmount).toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center pt-4 mt-2 border-t border-gray-100 dark:border-gray-700">
                       ...
                       <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400 tracking-tight">৳{selectedOrder.total.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white dark:bg-gray-800/50 p-6 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                  ...
                  <div className="space-y-5">
                    <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-800">
                      ...
                      <div className="flex items-center justify-between">
                         <span className="px-3 py-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-[10px] font-black uppercase text-gray-700 dark:text-gray-300 shadow-sm">{selectedOrder.paymentMethod}</span>
                         <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase shadow-sm ${selectedOrder.paymentStatus === 'PAID' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>{selectedOrder.paymentStatus}</span>
                      </div>
                    </div>
                      ...
                      <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-800 space-y-3">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center justify-between">
                          ...
                          {selectedOrder.trackingUrl && (
                            <a href={selectedOrder.trackingUrl} target="_blank" rel="noreferrer" className="text-emerald-500 hover:underline">Track</a>
                          )}
                        </p>
                    ...
                    {/* Cancel action */}
                    {selectedOrder.status !== 'CANCELLED' && selectedOrder.status !== 'RETURNED' && selectedOrder.status !== 'DELIVERED' && (
                      <button 
                ...
                <div className="bg-white dark:bg-gray-800/50 p-6 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col h-[400px]">
                  ...
                  <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pr-2 relative">
                    ...
                    <div className="space-y-6 relative z-10">
                      {((selectedOrder as any).orderNotes || []).map((note: any, index: number) => (
                        <div key={note.id} className="flex gap-4">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-sm border-2 border-white dark:border-gray-900 ${note.isSystem ? 'bg-gray-100 dark:bg-gray-800 text-gray-500' : 'bg-emerald-100 dark:bg-emerald-900 text-emerald-600 dark:text-emerald-400'}`}>
                            {note.isSystem ? <Settings size={12} /> : <Clock size={12} />}
                          </div>
                          <div className="pt-1 flex-1">
                            <p className="text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-widest">
                              {new Date(note.createdAt).toLocaleString()} 
                              {!note.isSystem && <span className="ml-2 text-emerald-500 bg-emerald-50 dark:bg-emerald-900/30 px-1.5 py-0.5 rounded-md">Note</span>}
                            </p>
                      ...
                      {((selectedOrder as any).orderNotes || []).length === 0 && (
                        <p className="text-xs text-gray-400 italic text-center py-4 bg-gray-50 dark:bg-gray-900 rounded-lg">No activity recorded yet.</p>
                      )}
                    </div>
                  </div>
          ...
          <div className="hidden print:block bg-white text-black p-8 font-sans w-full max-w-4xl mx-auto">
            ...
            <div className="flex justify-between items-start border-b-2 border-gray-900 pb-6 mb-8">
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-bold text-gray-800">Order #{selectedOrder.id}</p>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8">
              <div>
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Customer Info</h3>
                <p className="text-lg font-bold text-gray-900">{selectedOrder.customerName || selectedOrder.user?.name || "Guest"}</p>
                <p className="text-sm text-gray-600">{selectedOrder.customerPhone || selectedOrder.user?.phone || "No phone provided"}</p>
              </div>
              <div>
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Delivery Address</h3>
                <p className="text-sm text-gray-600 whitespace-pre-wrap leading-relaxed">{selectedOrder.deliveryAddress}</p>
              </div>
            </div>
                ...
                <tbody>
                  {selectedOrder.items.map((item, idx) => (
                    <tr key={idx} className="border-b border-gray-200">
                      ...
                      <td className="py-4 text-sm font-black text-gray-900 text-right">৳{(item.quantity * item.price).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex justify-end">
              <div className="w-1/2 space-y-3">
                ...
                <div className="flex justify-between items-center pt-3 border-t-2 border-gray-900 mt-3">
                  <span className="text-base font-black text-gray-900 uppercase tracking-widest">Grand Total</span>
            ...
            <div className="mt-16 pt-8 border-t border-gray-200 text-center">
              ...
              <p className="text-xs text-gray-500 mt-1">If you have any questions about this invoice, please contact support@femcart.com</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm transition-opacity duration-300 ${returnItem ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="absolute inset-0" onClick={() => !processingReturn && setReturnItem(null)} />
        <div className={`absolute right-0 top-0 bottom-0 w-full max-w-md bg-white dark:bg-gray-900 shadow-2xl border-l border-gray-200 dark:border-gray-800 transition-transform duration-500 ease-in-out flex flex-col ${returnItem ? 'translate-x-0' : 'translate-x-full'}`}>
          {returnItem && (
            <>
              <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900">
                <h3 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tight">Process Return</h3>
                <button onClick={() => setReturnItem(null)} className="p-2 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-full transition-colors"><X size={20} /></button>
              </div>
              ...
              <div className="p-6 flex-1 overflow-y-auto">
                <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30 rounded-lg mb-8">
                  <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-black uppercase tracking-widest mb-1">Target Product</p>
                  <p className="text-sm font-bold text-gray-900 dark:text-white">{returnItem.product?.name}</p>
                </div>

                <div className="space-y-6">
                  {/* Quantity */}
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Quantity to Return</label>
                    <div className="relative">
                      <input 
                        type="number" 
                        min="1" 
                        max={returnItem.quantity - ((returnItem as any).returnedQuantity || 0) - ((returnItem as any).damagedQuantity || 0)}
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Internal Reason (Optional)</label>
                    <textarea 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Refund Method</label>
                      <select 
                        value={refundMethod}
                        onChange={e => setRefundMethod(e.target.value as 'WALLET' | 'ORIGINAL' | 'MANUAL')}
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Shipping Fee</label>
                      <button 
              ...
              <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
                <div className="flex justify-between items-center mb-6 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Est. Refund</span>
                  <strong className="text-rose-500 text-2xl tracking-tight">
                    ৳{Math.round(((returnQuantity * returnItem.price * (1 - (selectedOrder.subtotal > 0 ? selectedOrder.discount / selectedOrder.subtotal : 0))) + (refundShipping ? selectedOrder.deliveryFee : 0)) * 100) / 100}
                  </strong>
                </div>
