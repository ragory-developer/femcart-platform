...
import { Calendar, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Package, MapPin, MessageSquare, CreditCard, Banknote } from "lucide-react";
        ...
        <table className="w-full text-left border-collapse min-w-[1000px] text-sm">
          <thead>
            <tr className="bg-gray-50/80 dark:bg-gray-800/80">
              <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Order ID & Date</th>
              <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Customer Details</th>
              <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Products</th>
              <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Total & Payment</th>
              ...
              <th className="px-6 py-3.5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider align-middle border border-gray-200 dark:border-gray-700">Details</th>
            </tr>
          </thead>
          <tbody>
            {orders.map((order) => (
              <tr key={order.id} className="group transition-colors duration-200 hover:bg-gray-50/80 dark:hover:bg-gray-750/30">
                <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750">
                  <div className="font-bold text-gray-900 dark:text-white break-all max-w-[120px] uppercase flex items-center gap-2">
                    {order.id}
                    {order.notes && <MessageSquare size={14} className="text-emerald-500 shrink-0" />}
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        ...
                        {order.user?.isGuest && <span className="text-[8px] font-black uppercase bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-md">GUEST</span>}
                      </p>
                      ...
                      {(order.deliveryArea || order.deliveryCity) && (
                        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-1 flex items-center gap-1">
                          <MapPin size={10} />
                          {[order.deliveryArea, order.deliveryCity].filter(Boolean).join(", ")}
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750">
                  <div className="flex flex-col gap-1 max-w-[200px]">
                    {order.items?.slice(0, 2).map((item, idx) => (
                      <p key={idx} className="text-xs font-bold text-gray-900 dark:text-white truncate" title={item.variant?.attributes?.length ? `${item.product?.name} (${item.variant.attributes.map((a: any) => a.value).join(' / ')})` : item.product?.name}>
                        <span className="text-emerald-600 dark:text-emerald-400 mr-1">{item.quantity}x</span> 
                ...
                <td className="px-6 py-4 align-middle border border-gray-200 dark:border-gray-750">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md ${
                      order.paymentStatus === 'PAID' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400'
                    }`}>
                      {order.paymentStatus}
                    </span>
                  </div>
