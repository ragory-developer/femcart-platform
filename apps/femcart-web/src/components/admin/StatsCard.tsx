  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-[clamp(1rem,3vw,1.5rem)] p-[clamp(1rem,3vw,1.5rem)] shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        <div className={`w-[clamp(2.5rem,6vw,3rem)] h-[clamp(2.5rem,6vw,3rem)] rounded-[clamp(0.75rem,2vw,1rem)] flex items-center justify-center ${colors.iconBg} ${colors.text}`}>
          {icon}
        </div>
        {trend && (
          <div className={`flex items-center gap-1 text-[clamp(0.65rem,1.5vw,0.75rem)] font-bold px-2 py-1 rounded-full ${
            trend.positive
      ...
      <p className="text-[clamp(1.5rem,5vw,2rem)] font-black text-gray-900 dark:text-white mb-1">{value}</p>
      <p className="text-[clamp(0.875rem,2vw,1rem)] text-gray-500 dark:text-gray-400 font-medium">{label}</p>
    </div>
  );
}
