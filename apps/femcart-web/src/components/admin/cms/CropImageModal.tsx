...
export function CropImageModal({ imageUrl, onClose, onSave }: { imageUrl: string, onClose: () => void, onSave: (url: string) => void }) {
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden ring-1 ring-white/10">
        ...
        <div className="p-5 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between bg-white dark:bg-gray-900">
           <div className="flex items-center gap-4 w-1/2">
             <span className="text-sm text-gray-500 dark:text-gray-400 font-medium">Zoom</span>
             <input
                type="range"
                value={zoom}
                min={1}
           ...
           <div className="flex gap-3">
             <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
               Cancel
             </button>
             <button 
               onClick={handleSave} 
