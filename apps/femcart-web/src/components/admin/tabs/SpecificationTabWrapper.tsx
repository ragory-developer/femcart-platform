...
export default function SpecificationTabWrapper() {
  ...
  const toggleSpecificationInProduct = (spec: Specification) => {
    ...
    if (idx !== -1) {
      ...
      // Add to product
      append({ name: spec.name, value: "", _attributeId: spec.id, _selectedValueIds: [] } as any);
    }
  ...
  const handleCreateSpecification = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        append({ name: created.name, value: "", _attributeId: created.id, _selectedValueIds: [] } as any);
        setNewSpecName("");
      } else {
  ...
  const handleDeleteSpecification = async (specId: string) => {
    if (!confirm("Are you sure you want to delete this specification globally?")) return;
    try {
  ...
  const toggleValue = (val: SpecificationValue) => {
    ...
    const idx = specifications.findIndex(a => a._attributeId === activeSpecId);
    if (idx !== -1) {
      // Update existing row
  ...
  const handleDeleteValue = async (val: SpecificationValue) => {
    if (!activeSpecId || !confirm(`Delete value "${val.value}" globally?`)) return;
    try {
    ...
    <div className="max-w-4xl space-y-8 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 md:p-8 shadow-sm">
        <div className="border-b border-gray-100 dark:border-gray-750 pb-4 mb-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Product Specifications</h2>
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingCatalog ? (
              ...
              filteredCatalog.map((spec) => {
                ...
                return (
                  <div key={spec.id} className={`group w-full flex items-center justify-between text-left transition-colors border-l-2 ${
                    isActive
                ...
                {activeSpecification.values.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {filteredValues.map((val) => {
                      const isSelected = activeSelectedIds.includes(val.id);
                      return (
                        <div key={val.id} className="relative group inline-block">
                          <button
