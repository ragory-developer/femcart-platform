...
function VariationSelector({
  ...
  return (
    <div ref={containerRef} className={`border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 transition-all ${typeOpen || valOpen ? 'relative z-50' : 'relative z-10'}`}>
            ...
            <div className="absolute left-0 top-full mt-1 w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl z-30 overflow-hidden">
              ...
              <div className="max-h-44 overflow-y-auto">
                {filteredTypes.length === 0 ? (
                  ...
                  >
                    <span>{v.name}</span>
                    {v.id === variationId && <Check size={13} className="text-emerald-500" />}
                  </button>
                ))}
              </div>
            </div>
              ...
              <div className="max-h-44 overflow-y-auto py-1">
                {filteredValues.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      <span>{val.value}</span>
                      {isOn && <Check size={14} className="text-emerald-600 dark:text-emerald-400" />}
                    </button>
                  );
                })}
              </div>
...
function VariantCard({ v, idx, expanded, hasError, onToggle, onUpdate, onRemove }: VariantCardProps) {
  ...
  return (
    <div className={`border rounded-xl overflow-hidden transition-all ${
      ...
      >
        ...
        >
          {v.image ? (
            <img src={resolveImageUrl(v.image)} alt="" className="w-full h-full object-cover" />
          ) : (
      ...
      {expanded && (
        <div className="border-t border-gray-100 dark:border-gray-700 px-4 py-4 space-y-4 bg-gray-50 dark:bg-gray-900/30">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Stock</label>
              <input type="number"
...
export default function VariationTab() {
  ...
  const { fields, append, remove: removeField, update: updateField } = useFieldArray({
    control,
    name: "variants"
  });
    ...
    <div className="max-w-4xl space-y-8 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 md:p-8 shadow-sm">
        <div className="border-b border-gray-100 dark:border-gray-750 pb-4 mb-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Product Variations</h2>
          <p className="text-sm text-gray-500 mt-1">Select variation types and values, then generate the combination rows.</p>
        </div>
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800">
        ...
        <div className="p-5 space-y-3">
          {selectors.map((sel, idx) => (
            <div key={idx} style={{ zIndex: 50 - idx }} className="relative">
              <VariationSelector
                catalog={catalog}
                loading={loading}
