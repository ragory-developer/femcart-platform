...
export const SearchSuggestionCard = React.memo(({ product, isSelected, onSelect }: SearchSuggestionCardProps) => {
  ...
  const displayBrand = product.highlights?.brandName?.[0] || product.brandName;
          ...
          <div className="flex items-center gap-1.5 shrink-0 pl-2">
            {product.specialPrice ? (
              <div className="flex flex-col items-end leading-tight">
                <span className="text-sm font-black text-rose-600 dark:text-rose-400">৳{Number(product.specialPrice).toFixed(2)}</span>
                <span className="text-[10px] font-medium text-gray-400 line-through">৳{Number(product.price).toFixed(2)}</span>
              </div>
            ) : (
              <span className="text-sm font-black text-gray-900 dark:text-white">৳{Number(product.price).toFixed(2)}</span>
            )}
          </div>
        </div>
