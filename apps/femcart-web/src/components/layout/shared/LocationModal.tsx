...
export function LocationModal({ isOpen, onClose, themeColor = "emerald" }: LocationModalProps) {
  const { settings, setSettings } = useSettingsStore();
  
  // Default to NYC or previous location
  const [position, setPosition] = useState<[number, number]>([
    settings.deliveryLat || 40.7128, 
    settings.deliveryLng || -74.0060
  ]);
  const [addressInput, setAddressInput] = useState(settings.deliveryLocation || "");
  ...
  useEffect(() => {
    ...
    if (position[0] === settings.deliveryLat && position[1] === settings.deliveryLng) {
      return;
    }
          ...
          <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gray-50/50">
            <div>
              <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <MapPin className="text-gray-500" size={20} /> Select Delivery Location
              </h2>
              <p className="text-sm text-gray-500 mt-1 font-medium">To see available items and exact pricing</p>
            </div>
            <button
              onClick={onClose}
