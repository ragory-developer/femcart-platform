...
import { Search, ShoppingCart, User as UserIcon, Menu, LayoutDashboard, LogOut, MapPin, Phone, Store, HelpCircle, ChevronDown, Mail, Globe } from "lucide-react";
...
const LocationModal = dynamic(() => import("@/components/layout/shared/LocationModal").then(mod => mod.LocationModal), { ssr: false });
...
export default function Navbar({
  ...
  const isHeroCategoryVisible = useNavigationStore(state => state.isHeroCategoryVisible);
                ...
                {topNavbarItems.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (hasDropdown) {
                          setHoveredCategory(hoveredCategory === item.id ? null : item.id);
                        }
                      }}
                    >
                      <Link
                      ...
                      >
                        {item.title}
                        {hasDropdown && (
                          <ChevronDown size={14} className={`text-white/70 transition-transform duration-200 ${hoveredCategory === item.id ? 'rotate-180 text-white' : ''}`} />
                        )}
                      </Link>
          ...
          <div className="max-w-[1600px] mx-auto px-4 md:px-6 flex justify-between items-start gap-2 md:gap-4 pt-1.5 pb-1 relative">
            ...
            <div className="shrink-0 hidden md:flex items-center text-[11px] font-bold text-gray-800 uppercase tracking-tight z-50 lg:w-[22%]">
              <div
                ...
                onMouseEnter={() => !isHeroCategoryVisible && setIsMegaMenuOpen(true)}
