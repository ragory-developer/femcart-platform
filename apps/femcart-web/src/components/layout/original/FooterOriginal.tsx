    ...
    >
      <div className="container mx-auto px-[clamp(1rem,4vw,2rem)]">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-[clamp(2rem,5vw,3rem)] mb-[clamp(2rem,6vw,4rem)]">
          ...
          <div className="space-y-6">
            <Link href="/" className="flex items-center gap-2 group z-50">
              {settings.store_logo ? (
                <Image unoptimized src={settings.store_logo} alt={settings.store_name || "Logo"} width={150} height={40} className="h-[clamp(2rem,5vw,2.5rem)] w-auto group-hover:scale-105 transition-transform duration-300" />
              ) : (
                <>
            ...
            <p className="text-white/80 leading-relaxed font-medium text-[clamp(0.875rem,1.5vw,1rem)]">
              {settings.footer_about_text || "We deliver the freshest groceries, organic vegetables, and daily essentials straight from farms to your home in just 30 minutes. Quality and freshness guaranteed."}
            </p>
            <div className="flex gap-[clamp(0.5rem,2vw,1rem)]">
              {settings.footer_facebook && (
                <a href={settings.footer_facebook} target="_blank" rel="noreferrer" className="w-[clamp(44px,10vw,48px)] h-[clamp(44px,10vw,48px)] min-w-[44px] min-h-[44px] rounded-full bg-white/10 flex items-center justify-center hover:bg-[#FACC15] hover:text-[#0F3A44] transition-colors">
                  <Facebook className="w-[clamp(18px,4vw,20px)] h-[clamp(18px,4vw,20px)]" />
                </a>
              )}
              {settings.footer_twitter && (
                <a href={settings.footer_twitter} target="_blank" rel="noreferrer" className="w-[clamp(44px,10vw,48px)] h-[clamp(44px,10vw,48px)] min-w-[44px] min-h-[44px] rounded-full bg-white/10 flex items-center justify-center hover:bg-[#FACC15] hover:text-[#0F3A44] transition-colors">
                  <Twitter className="w-[clamp(18px,4vw,20px)] h-[clamp(18px,4vw,20px)]" />
                </a>
              )}
              {settings.footer_instagram && (
                <a href={settings.footer_instagram} target="_blank" rel="noreferrer" className="w-[clamp(44px,10vw,48px)] h-[clamp(44px,10vw,48px)] min-w-[44px] min-h-[44px] rounded-full bg-white/10 flex items-center justify-center hover:bg-[#FACC15] hover:text-[#0F3A44] transition-colors">
          ...
          {footerSections
            ...
            .map((sec: any) => (
            <div key={sec.id}>
              <h4 className="text-white font-bold font-display tracking-widest uppercase text-[clamp(1.125rem,2vw,1.25rem)] mb-[clamp(1rem,3vw,1.5rem)]">{sec.title}</h4>
              <ul className="space-y-[clamp(0.5rem,1.5vw,1rem)] font-medium text-white/70">
                {sec.links?.filter((link: any) => link.isActive).map((link: any) => (
                  <li key={link.id}>
                    <Link href={link.url ? (link.url.startsWith('http') || link.url.startsWith('#') || link.url.startsWith('/') ? link.url : `/${link.url}`) : "#"} target={link.target || "_self"} prefetch={false} className="hover:text-[#FACC15] transition-colors inline-block py-1">
                      {link.title}
                    </Link>
                  </li>
                ))}
          ...
          {/* Contact Info */}
          <div>
            <h4 className="text-white font-bold font-display tracking-widest uppercase text-[clamp(1.125rem,2vw,1.25rem)] mb-[clamp(1rem,3vw,1.5rem)]">Contact Us</h4>
            <ul className="space-y-[clamp(1rem,2vw,1.25rem)] text-[clamp(0.875rem,1.5vw,1rem)]">
              <li className="flex gap-3 text-white/80">
                ...
                <span className="font-medium leading-relaxed whitespace-pre-line">
                  {contactAddress || settings.footer_address || "123 Femcart Avenue, Suite 400\nNew York, NY 10001"}
                </span>
              </li>
              <li className="flex gap-3 text-white/80 items-center">
                ...
                <span className="font-medium">{contactPhone || settings.footer_phone || "+1 (800) 123-4567"}</span>
              </li>
              <li className="flex gap-3 text-white/80 items-center">
                <Mail className="text-[#FACC15] shrink-0 w-[clamp(18px,4vw,20px)] h-[clamp(18px,4vw,20px)]" />
                <span className="font-medium truncate">{contactEmail || settings.footer_email || "support@femcart.test"}</span>
              </li>
            </ul>
          </div>
        ...
        <div className="pt-8 border-t border-gray-800 text-center md:flex md:justify-between md:text-left text-gray-500 text-[clamp(0.75rem,1.5vw,0.875rem)] font-medium flex flex-col md:flex-row gap-4 items-center">
          <p>&copy; {new Date().getFullYear()} {settings.footer_copyright || "Femcart. All rights reserved."}</p>
