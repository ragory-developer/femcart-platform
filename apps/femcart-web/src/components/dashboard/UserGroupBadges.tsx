    ...
    <div className="mt-[clamp(2rem,6vw,4rem)] relative">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-[clamp(1.5rem,4vw,2.5rem)] gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium text-[clamp(0.875rem,2vw,1rem)] mt-1">Unlock premium benefits as you shop more</p>
        </div>
        <div className="flex items-center gap-2 bg-white dark:bg-gray-900 px-[clamp(0.75rem,2vw,1rem)] py-[clamp(0.5rem,1.5vw,0.75rem)] min-h-[44px] rounded-full border border-gray-100 dark:border-gray-800 shadow-sm w-fit">
          <span className="text-[clamp(0.875rem,2vw,1rem)] font-black text-blue-600">{purchaseCount}</span>
          <span className="text-[clamp(0.65rem,1.5vw,0.75rem)] font-bold text-gray-500 uppercase tracking-widest">Total Purchases</span>
        </div>
      </div>
        ...
        {groups.map((group, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Tier Badge / Circle */}
              <div className={`relative w-[clamp(4rem,10vw,5rem)] h-[clamp(4rem,10vw,5rem)] shrink-0 rounded-[clamp(1rem,3vw,1.5rem)] flex items-center justify-center transition-all duration-500 shadow-xl
                ${isActive 
              ...
              {/* Content Card */}
              <div className={`ml-[clamp(1rem,3vw,2rem)] flex-grow p-[clamp(1rem,3vw,1.5rem)] rounded-[clamp(1.5rem,4vw,2rem)] border transition-all duration-500
                ${isActive 
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h4 className={`text-[clamp(1.125rem,3vw,1.25rem)] font-black tracking-tight ${isActive ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}>
                        {group.name}
                      </h4>
                      {isActive && (
                    ...
                    <p className={`text-[clamp(0.75rem,2vw,0.875rem)] font-bold uppercase tracking-widest mb-[clamp(0.25rem,1vw,0.5rem)] ${isActive ? 'text-blue-500' : 'text-gray-400'}`}>
                  ...
                  {!isUnlocked && (
                    <div className="md:text-right">
                       <p className="text-[clamp(0.65rem,1.5vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest mb-[clamp(0.125rem,0.5vw,0.25rem)]">Locked</p>
