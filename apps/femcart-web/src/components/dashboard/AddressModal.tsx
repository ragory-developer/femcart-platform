...
export default function AddressModal({ address, onClose, onSuccess }: AddressModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        toast.success(address?.id ? "Address updated successfully" : "Address saved successfully");
        onSuccess();
        onClose();
      } else {
      ...
      Logger.error("Failed to save address due to network error", err, "AddressModal");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white dark:bg-gray-950 rounded-[clamp(1.5rem,4vw,2.5rem)] w-full max-w-lg shadow-2xl relative my-[clamp(1rem,3vw,2rem)] animate-in fade-in zoom-in duration-300">
        ...
        <form onSubmit={handleSubmit} className="p-[clamp(1.5rem,4vw,2.5rem)]">
          ...
          <div className="space-y-4">
            {/* Label */}
            <div>
              <label className="block text-[clamp(0.65rem,1.5vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest mb-[clamp(0.25rem,1vw,0.375rem)] ml-1">Address Label</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[clamp(0.65rem,1.5vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest mb-[clamp(0.25rem,1vw,0.375rem)] ml-1">Contact Name</label>
              ...
              <div>
                <label className="block text-[clamp(0.65rem,1.5vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest mb-[clamp(0.25rem,1vw,0.375rem)] ml-1">Phone Number</label>
                <div className="relative">
            ...
            {/* Address */}
            <div>
              <label className="block text-[clamp(0.65rem,1.5vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest mb-[clamp(0.25rem,1vw,0.375rem)] ml-1">Full Address</label>
              <textarea
                required
                rows={3}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[clamp(0.65rem,1.5vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest mb-[clamp(0.25rem,1vw,0.375rem)] ml-1">City</label>
                <input
                  type="text"
                  required
                  value={formData.city}
              ...
              <div>
                <label className="block text-[clamp(0.65rem,1.5vw,0.75rem)] font-black text-gray-400 uppercase tracking-widest mb-[clamp(0.25rem,1vw,0.375rem)] ml-1">Area</label>
                <input
                  type="text"
                  required
                  value={formData.area}
            ...
            <label className="flex items-center gap-[clamp(0.5rem,1.5vw,0.75rem)] cursor-pointer group mt-[clamp(0.5rem,1.5vw,1rem)] min-h-[44px]">
              ...
              />
              <span className="text-[clamp(0.875rem,2vw,1rem)] font-bold text-gray-600 dark:text-gray-400 group-hover:text-blue-600 transition-colors">Set as default address</span>
            </label>
          </div>
