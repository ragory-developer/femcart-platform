...
import { Info, ListChecks, MessageCircleQuestion, Star, ChevronDown } from "lucide-react";
...
export default function ProductTabs({ product }: { product: any }) {
  ...
  const validSpecifications = (product.specifications && Array.isArray(product.specifications))
  ...
  const validFaqs = (product.faqs && Array.isArray(product.faqs))
    ? product.faqs.filter((faq: any) => faq && faq.question && faq.question.trim() !== "" && faq.answer && faq.answer.trim() !== "")
    : [];
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden mt-[clamp(1.5rem,4vw,3rem)]">
      ...
      <div className="p-4 sm:p-[clamp(1rem,4vw,2rem)] min-h-[250px] sm:min-h-[300px]">
        {/* Description Tab */}
        <div className={activeTab === "description" ? "block animate-in fade-in slide-in-from-bottom-2 duration-500" : "hidden"}>
          <div className="text-gray-600 dark:text-gray-300">
            {product.description ? (
              ...
              <div className="py-8 text-center bg-gray-50/50 dark:bg-gray-800/30 rounded-xl border border-gray-100 dark:border-gray-800">
                ...
                <p className="text-gray-500 dark:text-gray-400 font-medium tracking-tight">No detailed description available for this product.</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Specifications Tab */}
        <div className={activeTab === "specifications" ? "block animate-in fade-in slide-in-from-bottom-2 duration-500" : "hidden"}>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {product.weight && (
                  <tr className="even:bg-gray-50/50 dark:even:bg-gray-800/30 hover:bg-gray-50/80 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.5rem,1.5vw,1rem)] font-bold w-1/3 text-gray-900 dark:text-white capitalize">Weight / Volume</td>
                    <td className="py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.5rem,1.5vw,1rem)] font-medium">{product.weight} {product.unit}</td>
                  </tr>
                )}
                
                {product.categories && product.categories.length > 0 && (
                  <tr className="even:bg-gray-50/50 dark:even:bg-gray-800/30 hover:bg-gray-50/80 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.5rem,1.5vw,1rem)] font-bold w-1/3 text-gray-900 dark:text-white capitalize">Categories</td>
                    <td className="py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.5rem,1.5vw,1rem)] font-medium">{product.categories.map((c: any) => c.name).join(", ")}</td>
                  </tr>
                )}

                {product.brand && (
                  <tr className="even:bg-gray-50/50 dark:even:bg-gray-800/30 hover:bg-gray-50/80 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.5rem,1.5vw,1rem)] font-bold w-1/3 text-gray-900 dark:text-white capitalize">Brand</td>
                    <td className="py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.5rem,1.5vw,1rem)] font-medium">{product.brand.name}</td>
                  </tr>
                )}

                {validSpecifications.length > 0 && validSpecifications.map((s: any, i: number) => (
                  <tr key={`spec-${i}`} className="even:bg-gray-50/50 dark:even:bg-gray-800/30 hover:bg-gray-50/80 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.5rem,1.5vw,1rem)] font-bold w-1/3 text-gray-900 dark:text-white capitalize">{s.name}</td>
                    <td className="py-[clamp(0.75rem,2vw,1rem)] px-[clamp(0.5rem,1.5vw,1rem)] font-medium">
                      <div className="flex flex-wrap gap-2">
                        {s.value ? s.value.split(" | ").map((v: string) => (
                          <span key={v} className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 px-3 py-1 rounded-full text-[clamp(0.75rem,1.5vw,0.875rem)] border border-gray-200 dark:border-gray-700 shadow-sm">{v}</span>
                        )) : "-"}
                      </div>
                    </td>
                  </tr>
        ...
        {/* Questions Tab (FAQs) */}
        <div className={activeTab === "questions" ? "block animate-in fade-in slide-in-from-bottom-2 duration-500" : "hidden"}>
          <div className="space-y-4">
            {hasFaqs && (
              <div className="grid grid-cols-1 gap-3" itemScope itemType="https://schema.org/FAQPage">
                        ...
                        {isOpen && (
                          ...
                          >
                            <div className="px-[clamp(1rem,3vw,1.5rem)] pb-[clamp(1rem,3vw,1.5rem)] pt-0 ml-[52px]">
                              <div className="text-gray-600 dark:text-gray-300 leading-relaxed font-medium bg-white dark:bg-gray-900 p-[clamp(1rem,2vw,1.25rem)] rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm" itemProp="suggestedAnswer acceptedAnswer" itemScope itemType="https://schema.org/Answer">
        ...
        <div className={activeTab === "reviews" ? "block animate-in fade-in slide-in-from-bottom-2 duration-500" : "hidden"}>
          ...
          <div className="flex items-center gap-[clamp(1rem,3vw,1.5rem)] mb-[clamp(1.5rem,4vw,2rem)] p-[clamp(1rem,3vw,1.5rem)] bg-pink-50 dark:bg-pink-900/20 rounded-lg">
            <div className="text-center">
              ...
              <div className="text-[clamp(0.75rem,1.5vw,0.875rem)] text-gray-500">Based on {product.ratingCount || 0} reviews</div>
            </div>
