...
import CommerceCarousel, { CommerceCarouselConfig } from "@/components/ui/carousel/CommerceCarousel";
...
export default function ProductReviews({
  ...
  const sectionTitle = title || (isProductMode ? "Customer Reviews" : "Real Results, Real Beauty");
  const sectionSubtitle = subtitle || (isProductMode 
  ...
  const dbReviewsStr = productContext?.reviews ? JSON.stringify(productContext.reviews) : "";
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (isProductMode) {
          if (productId === "dummy-1") {
            ...
            if (isMounted) {
              setItems([
                { id: "d1", name: "Sarah J.", avatar: "", rating: 5, review: "Absolutely love the freshness of these avocados! Perfect for my morning toast.", createdAt: new Date().toISOString(), product: "Premium Organic Avocado Box" },
                { id: "d2", name: "Michael T.", avatar: "", rating: 4, review: "Great quality, but one was a bit too ripe. Still delicious though.", createdAt: new Date().toISOString(), product: "Premium Organic Avocado Box" },
                { id: "d3", name: "Emma W.", avatar: "", rating: 5, review: "Best produce I've ever ordered online. Will definitely buy again.", createdAt: new Date().toISOString(), product: "Premium Organic Avocado Box" }
              ].slice(0, limit));
              setLoading(false);
            }
            return;
          ...
          if (dbReviews && Array.isArray(dbReviews) && dbReviews.length > 0) {
            if (isMounted) {
              setItems(dbReviews.slice(0, limit).map((r: any, idx: number) => ({
                id: r.id || `db-r-${idx}`,
          ...
          if (productId) {
            const res = await fetch(`${API_URL}/api/reviews/product/${productId}?limit=${limit}`);
            if (res.ok) {
              const data = await res.json();
              if (data.success && data.data.length > 0) {
                if (isMounted) {
                  setItems(data.data.map((r: any) => ({
                    id: r.id,
                    name: r.reviewer,
                    avatar: r.avatar || "",
                    rating: r.rating || 5,
          ...
          const parsedTestimonials = testimonialsStr ? JSON.parse(testimonialsStr) : null;
          ...
          const res = await fetch(`${API_URL}/api/reviews?limit=${limit}&featured=true`);
          if (res.ok) {
            const data = await res.json();
            if (data.success && data.data.length > 0) {
              if (isMounted) {
    ...
    }, [isProductMode, productId, productName, dbReviewsStr, testimonialsStr, limit]);


  const hasBuilderPadding = builderClassName?.includes("p-") || builderClassName?.includes("py-") || builderStyle?.padding;
  const hasBuilderBg = builderClassName?.includes("bg-") || builderStyle?.backgroundColor || builderStyle?.backgroundImage;
  const hasCustomColor = !!builderStyle?.color || builderClassName?.includes("text-");

  const titleColorClass = hasCustomColor ? "text-inherit" : "text-gray-900 dark:text-white";
  const subtitleColorClass = hasCustomColor ? "text-inherit opacity-80" : "text-gray-500 dark:text-gray-400";
    ...
    >
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12">
          <span className={`inline-flex items-center gap-2 px-[clamp(0.75rem,2vw,1rem)] py-[clamp(0.25rem,1vw,0.375rem)] rounded-full text-[clamp(0.75rem,1.5vw,0.875rem)] font-bold mb-4 ${styles.badgeBg}`}>
            <Star size={14} className={styles.starColor} /> {isProductMode ? "Customer Reviews" : "Customer Reviews"}
          </span>
          <h2 className={`text-[clamp(1.5rem,4vw,2.5rem)] font-black mb-[clamp(0.5rem,2vw,0.75rem)] ${titleColorClass}`}>
            {sectionTitle}
          </h2>
          <p className={`text-[clamp(0.875rem,2vw,1.125rem)] max-w-lg mx-auto ${subtitleColorClass}`}>
            {sectionSubtitle}
          </p>
        </div>
