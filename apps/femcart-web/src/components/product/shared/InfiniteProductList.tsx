...
export default function InfiniteProductList({
  ...
  if (!enabled) {
    return (
      <div className={`-mx-2 sm:mx-0 grid grid-cols-2 md:grid-cols-2 ${gridCols} gap-2 sm:gap-4 lg:gap-[clamp(1rem,3vw,1.5rem)]`}>
...
function EmptyState() {
  return (
    <div className="col-span-full py-[clamp(4rem,10vw,8rem)] text-center bg-white dark:bg-gray-900 rounded-[32px] border border-orange-200/40 shadow-sm">
      ...
      <h3 className="text-[clamp(1.25rem,4vw,1.5rem)] font-black text-gray-900 dark:text-white mb-2 uppercase italic tracking-tighter">No items found</h3>
      <p className="text-[clamp(0.875rem,2vw,1rem)] text-gray-500 max-w-xs mx-auto font-medium">Try adjusting your filters or search criteria to find what you're looking for.</p>
      <Link href="/products" className="inline-block flex items-center justify-center mx-auto w-fit mt-8 bg-pink-500 text-white px-[clamp(1.5rem,4vw,2rem)] py-[clamp(0.75rem,2vw,1rem)] min-h-[48px] rounded-full font-black text-[clamp(0.875rem,2vw,1rem)] uppercase tracking-widest hover:bg-pink-600 transition-all shadow-lg">
         Reset All Filters
      </Link>
    </div>
  );
