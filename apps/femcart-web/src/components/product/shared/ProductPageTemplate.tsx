...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const cleanDescription = (seo?.description || shortDescription || description || `Buy ${name} online at Femcart. Best quality guaranteed.`)
    .replace(/<[^>]*>/g, " ")
    .replace(/\s+/g, " ")
    .trim()
    .substring(0, 160);
  ...
  const keywords = seo?.keywords || (categories?.[0]?.name ? `${name}, ${categories[0].name}, Femcart` : `${name}, Femcart`);
      ...
      <div className="container mx-auto px-4 py-4 md:py-6 lg:py-8">
        <nav className="hidden md:flex items-center mb-6 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide text-xs font-bold text-gray-700">
          {[
            ...
            ...(data.categories?.[0] ? [{ label: data.categories[0].name, href: `/categories/${data.categories[0].slug}` }] : []),
            { label: name, href: null }
          ].map((item, index) => (
            <div key={index} className={`flex relative ${['z-40', 'z-30', 'z-20', 'z-10'][index] || 'z-0'} ${index !== 0 ? '-ml-2' : ''}`}>
              {item.href ? (
                <Link 
                  href={item.href}
