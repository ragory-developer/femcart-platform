...
export default function ProductFilters({ onClose, hideCategoryFilter = false }: { onClose?: () => void, hideCategoryFilter?: boolean }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const pathname = usePathname();
  const { filters, applyFilters, toggleBrand, toggleCategory, toggleAttribute, setPriceRange, setMinRating } = useProductFilters();

  const scrollableClass = "max-h-60 overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-200 dark:[&::-webkit-scrollbar-thumb]:bg-gray-800 [&::-webkit-scrollbar-thumb]:rounded-full";
  ...
  console.log("DEBUG ProductFilters: filters.minPrice =", filters.minPrice, "localMinPrice =", localMinPrice);
  ...
  }, [localMinPrice, localMaxPrice, filters.minPrice, filters.maxPrice, setPriceRange]);
  ...
  useEffect(() => {
    const fetchDynamicFilters = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/api/products/filters${query ? `?${query}` : ''}`);
  ...
  if (loading && !filterData.categories.length) {
    return (
      <div className="animate-pulse space-y-4 pr-4">
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} className="h-16 bg-gray-100 dark:bg-gray-800 w-full rounded-lg" />
        ))}
      </div>
    );
  }
  ...
  const SectionHeader = ({ title, section, isExpanded }: { title: string; section: string; isExpanded: boolean }) => (
    <button 
    ...
    >
      ...
      <ChevronDown 
        ...
        className={`transform transition-transform duration-300 ${
          isExpanded ? 'rotate-180 text-emerald-500' : 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300'
        }`} 
      />
    </button>
  );
  ...
  return (
    <div className="flex flex-col w-full shrink-0 pb-12 lg:pb-0 font-sans">
      ...
      {!hideCategoryFilter && categories.length > 0 && (
        <div className={sectionWindowClass}>
          <SectionHeader title="Categories" section="categories" isExpanded={expandedSections.categories} />
              ...
              >
                <div className={`pt-4 pb-1 space-y-3.5 ${scrollableClass}`}>
                  {categories.map((cat: any) => {
                    ...
                    return (
                      <label key={cat.id} className="flex items-center justify-between cursor-pointer group" onClick={() => toggleCategory(cat.slug)}>
                        <div className="flex items-center gap-3">
                          <div className={`w-[18px] h-[18px] rounded-md border flex items-center justify-center shrink-0 transition-all duration-200 ${
                            selected 
                          ...
                          <span className={`text-[13px] font-medium transition-colors duration-200 ${
                            selected ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
                          }`}>
                            {cat.name}
                          </span>
                        </div>
      ...
      {brands.length > 0 && (
        <div className={sectionWindowClass}>
          <SectionHeader title="Brands" section="brands" isExpanded={expandedSections.brands} />
          <AnimatePresence initial={false}>
              ...
              >
                <div className={`pt-4 pb-1 space-y-3.5 ${scrollableClass}`}>
                  {brands.map((brand: any) => {
                    ...
                    return (
                      <label key={brand.id} className="flex items-center justify-between cursor-pointer group" onClick={() => toggleBrand(brand.slug)}>
                        <div className="flex items-center gap-3">
                          <div className={`w-[18px] h-[18px] rounded-md border flex items-center justify-center shrink-0 transition-all duration-200 ${
                            selected 
                              ? 'border-emerald-500 bg-emerald-500 text-white shadow-sm shadow-emerald-500/10' 
      ...
      {specifications.map((spec: any) => {
        ...
        return (
          <div key={spec.name} className={sectionWindowClass}>
            <SectionHeader title={spec.name} section={spec.name} isExpanded={expandedSections[spec.name]} />
            <AnimatePresence initial={false}>
              {expandedSections[spec.name] && (
                <motion.div
                  ...
                  className="overflow-hidden"
                >
                  <div className={`pt-4 pb-1 ${_isColor ? 'flex flex-wrap gap-3' : _isSize ? 'flex flex-wrap gap-2.5' : `space-y-3.5 ${scrollableClass}`}`}>
                    {spec.values.map((v: any) => {
                      const selected = (filters.attributes[spec.name] || []).includes(v.value);
                      ...
                      if (_isColor) {
                        return (
                          ...
                          >
                            {selected && <Check size={12} className={['white', 'light', 'transparent', 'yellow', 'beige'].some(c => v.value.toLowerCase().includes(c)) ? 'text-gray-900' : 'text-white'} />}
                          </button>
                        );
                      }
                      ...
                      return (
                        <label key={v.value} className="flex items-center justify-between cursor-pointer group">
                          <div className="flex items-center gap-3">
                            <div className={`w-[18px] h-[18px] rounded-md border flex items-center justify-center shrink-0 transition-all duration-200 ${
                              selected 
                                ? 'border-emerald-500 bg-emerald-500 text-white shadow-sm shadow-emerald-500/10' 
      ...
      {/* Price Range Window */}
      <div className={sectionWindowClass}>
        <SectionHeader title="Price" section="price" isExpanded={expandedSections.price} />
        <AnimatePresence initial={false}>
              ...
              <div className="pt-6 pb-1 space-y-6">
                <div className="px-3">
                  <Slider
                    range
                    min={priceRange.min}
                    max={priceRange.max > priceRange.min ? priceRange.max : 50000}
                    step={10}
                    value={[Number(localMinPrice) || priceRange.min, Number(localMaxPrice) || priceRange.max]}
                    onChange={(val: any) => {
                    ...
                    handleStyle={[
                      { borderColor: "#10b981", backgroundColor: "#fff", opacity: 1, boxShadow: "0 0 0 4px rgba(16,185,129,0.1)" },
                      { borderColor: "#10b981", backgroundColor: "#fff", opacity: 1, boxShadow: "0 0 0 4px rgba(16,185,129,0.1)" }
                    ]}
      ...
      <div className={sectionWindowClass}>
        <SectionHeader title="Rating" section="rating" isExpanded={expandedSections.rating} />
        <AnimatePresence initial={false}>
            ...
            >
              <div className="pt-4 pb-1 space-y-3.5">
                {[5, 4, 3, 2, 1].map((rating) => {
                  ...
                  return (
                    <label key={rating} className="flex items-center justify-between cursor-pointer group" onClick={(e) => {
                      e.preventDefault();
                      setMinRating(selected ? '' : rating.toString());
                    }}>
                      <div className="flex items-center gap-3">
                        <div className={`w-[18px] h-[18px] rounded-full flex items-center justify-center shrink-0 transition-all duration-200 border ${
                          selected ? 'border-emerald-500 p-1 bg-emerald-500 text-white shadow-sm shadow-emerald-500/10' : 'border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 group-hover:border-emerald-500'
                        }`}>
                        ...
                        <div className="flex items-center gap-0.5">
                          {[...Array(5)].map((_, i) => (
                            <Star 
                              key={i} 
                              size={13} 
                              className={i < rating ? "fill-amber-400 text-amber-400 animate-pulse-subtle" : "fill-gray-100 text-gray-100 dark:fill-gray-800 dark:text-gray-800"} 
                            />
                          ))}
                          <span className={`ml-2 text-[13px] font-medium transition-colors duration-200 ${
                            selected ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
                          }`}>
                            & Up
                          </span>
                        </div>
