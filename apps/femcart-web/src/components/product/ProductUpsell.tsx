...
export default function ProductUpsell({
  ...
  const items = useMemo(() => {
    ...
    let resolved = [];
    if (productContext?.resolvedUpsells && Array.isArray(productContext.resolvedUpsells) && productContext.resolvedUpsells.length > 0) {
    ...
    else if (productContext?.upsellProductIds && Array.isArray(productContext.upsellProductIds) && allProducts.length > 0) {
    ...
    return (
      ...
      >
        <div className="w-full border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-12 flex flex-col items-center justify-center text-center bg-gray-50/50 dark:bg-gray-900/50">
          <p className="text-gray-500 font-medium">No upsell products selected</p>
          <p className="text-sm text-gray-400 mt-1">Configure upsell (you may also like) products in the product settings to display them here.</p>
        </div>
      </SectionWrapper>
    );
  }
